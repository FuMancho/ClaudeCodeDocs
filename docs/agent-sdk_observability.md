...
For the complete list of metric names, event names, and attributes, see the Claude Code [Monitoring](./monitoring-usage.md "/docs/en/monitoring-usage") reference. The Agent SDK emits the same data because it runs the same CLI. Span names are listed in [Read agent traces](#read-agent-traces "#read-agent-traces") below.
...
### [​](#flush-telemetry-from-short-lived-calls "#flush-telemetry-from-short-lived-calls") Flush telemetry from short-lived calls
...
* **`claude_code.hook`:** wraps each [hook](./agent-sdk/hooks.md "/docs/en/agent-sdk/hooks") execution. Requires detailed beta tracing (`ENABLE_BETA_TRACING_DETAILED=1` and `BETA_TRACING_ENDPOINT`) in addition to the variables above.
...
Leave these unset unless your observability pipeline is approved to store the data your agent handles. See [Security and privacy](./monitoring-usage.md#security-and-privacy "/docs/en/monitoring-usage#security-and-privacy") in the Monitoring reference for the full list of attributes and redaction behavior.
...
* [Track cost and usage](./agent-sdk/cost-tracking.md "/docs/en/agent-sdk/cost-tracking"): read token and cost data from the message stream without an external backend.
* [Hosting the Agent SDK](./agent-sdk/hosting.md "/docs/en/agent-sdk/hosting"): deploy agents in containers where you can set OpenTelemetry variables at the environment level.
* [Monitoring](./monitoring-usage.md "/docs/en/monitoring-usage"): the complete reference for every environment variable, metric, and event the CLI emits.