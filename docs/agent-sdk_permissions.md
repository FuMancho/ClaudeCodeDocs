...
Check `ask` rules from [settings.json](./settings.md#permission-settings). If an ask rule matches, the call falls through to your [`canUseTool` callback](./agent-sdk_user-input.md) for confirmation, even in `bypassPermissions` mode. In `dontAsk` mode a matching ask rule is denied instead, because that mode never prompts.
