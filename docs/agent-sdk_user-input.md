...
1. **Tool needs approval**: Claude wants to use a tool that isn’t auto-approved by [permission rules](./agent-sdk/permissions.md "/docs/en/agent-sdk/permissions") or modes. Check `tool_name` for the tool (e.g., `"Bash"`, `"Write"`).
...
| `options` (TS) / `context` (Python) | Additional context including optional `suggestions` (proposed `PermissionUpdate` entries to avoid re-prompting) and a cancellation signal. In TypeScript, `signal` is an `AbortSignal`; in Python, the signal field is reserved for future use. See [`ToolPermissionContext`](./agent-sdk/python.md#toolpermissioncontext "/docs/en/agent-sdk/python#toolpermissioncontext") for Python. |
...
See the SDK reference for complete input schemas: [Python](./agent-sdk/python.md#tool-input%2Foutput-types "/docs/en/agent-sdk/python#tool-input%2Foutput-types") | [TypeScript](./agent-sdk/typescript.md#tool-input-types "/docs/en/agent-sdk/typescript#tool-input-types").
...
In Python, `can_use_tool` requires [streaming mode](./agent-sdk/streaming-vs-single-mode.md "/docs/en/agent-sdk/streaming-vs-single-mode") and a `PreToolUse` hook that returns `{"continue_": True}` to keep the stream open. Without this hook, the stream closes before the permission callback can be invoked.
...
### [​](#respond-to-tool-requests "#respond-to-tool-requests") Respond to tool requests
...
* **Redirect entirely**: use [streaming input](./agent-sdk/streaming-vs-single-mode.md "/docs/en/agent-sdk/streaming-vs-single-mode") to send Claude a completely new instruction
...
For a complete change of direction (not just a nudge), use [streaming input](./agent-sdk/streaming-vs-single-mode.md "/docs/en/agent-sdk/streaming-vs-single-mode") to send Claude a new instruction directly. This bypasses the current tool request and gives Claude entirely new instructions to follow.
...
Clarifying questions are especially common in [`plan` mode](./agent-sdk/permissions.md#plan-mode-plan "/docs/en/agent-sdk/permissions#plan-mode-plan"), where Claude explores the codebase and asks questions before proposing a plan. This makes plan mode ideal for interactive workflows where you want Claude to gather requirements before making changes.
...
See [Question format](#question-format "#question-format") for full field descriptions.
...
For multi-select questions, pass an array of labels or join them with `", "`. If you [support free-text input](#support-free-text-input "#support-free-text-input"), use the user’s custom text as the value.
...
#### [​](#option-previews-typescript "#option-previews-typescript") Option previews (TypeScript)
...
#### [​](#support-free-text-input "#support-free-text-input") Support free-text input
...
See the [complete example](#complete-example "#complete-example") below for a full implementation.
...
## [​](#other-ways-to-get-user-input "#other-ways-to-get-user-input") Other ways to get user input
...
Use [streaming input](./agent-sdk/streaming-vs-single-mode.md "/docs/en/agent-sdk/streaming-vs-single-mode") when you need to:
...
Use [custom tools](./agent-sdk/custom-tools.md "/docs/en/agent-sdk/custom-tools") when you need to:
...
* [Configure permissions](./agent-sdk/permissions.md "/docs/en/agent-sdk/permissions"): set up permission modes and rules
* [Control execution with hooks](./agent-sdk/hooks.md "/docs/en/agent-sdk/hooks"): run custom code at key points in the agent lifecycle
* [TypeScript SDK reference](./agent-sdk/typescript.md#canusetool "/docs/en/agent-sdk/typescript#canusetool"): full canUseTool API documentation