...
After [installing Claude Code](./setup.md#install-claude-code "/docs/en/setup#install-claude-code"), run `claude` in your terminal. On first launch, Claude Code opens a browser window for you to log in.
...
* **Cloud providers**: if your organization uses [Amazon Bedrock](./amazon-bedrock.md "/docs/en/amazon-bedrock"), [Google Vertex AI](./google-vertex-ai.md "/docs/en/google-vertex-ai"), or [Microsoft Foundry](./microsoft-foundry.md "/docs/en/microsoft-foundry"), set the required environment variables before running `claude`. No browser login is needed.

To log out and re-authenticate, type `/logout` at the Claude Code prompt.
If you’re having trouble logging in, see [authentication troubleshooting](./troubleshoot-install.md#login-and-authentication "/docs/en/troubleshoot-install#login-and-authentication").
...
* [Check system requirements](./setup.md#system-requirements "/docs/en/setup#system-requirements")
* [Install Claude Code](./setup.md#install-claude-code "/docs/en/setup#install-claude-code")
...
Follow the [Bedrock docs](./amazon-bedrock.md "/docs/en/amazon-bedrock"), [Vertex docs](./google-vertex-ai.md "/docs/en/google-vertex-ai"), or [Microsoft Foundry docs](./microsoft-foundry.md "/docs/en/microsoft-foundry").

2

Distribute configuration

Distribute the environment variables and instructions for generating cloud credentials to your users. Read more about how to [manage configuration here](./settings.md "/docs/en/settings").

3

Install Claude Code

Users can [install Claude Code](./setup.md#install-claude-code "/docs/en/setup#install-claude-code").
...
* **Storage location**:
  ...
  + Claude Code manages `.credentials.json` through `/login` and `/logout`. To route requests through a custom API endpoint, set the [`ANTHROPIC_BASE_URL`](./env-vars.md "/docs/en/env-vars") environment variable instead.
* **Supported authentication types**: Claude.ai credentials, Claude API credentials, Azure Auth, Bedrock Auth, and Vertex Auth.
* **Custom credential scripts**: the [`apiKeyHelper`](./settings.md#available-settings "/docs/en/settings#available-settings") setting can be configured to run a shell script that returns an API key.
...
1. Cloud provider credentials, when `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`, or `CLAUDE_CODE_USE_FOUNDRY` is set. See [third-party integrations](./third-party-integrations.md "/docs/en/third-party-integrations") for setup.
2. `ANTHROPIC_AUTH_TOKEN` environment variable. Sent as the `Authorization: Bearer` header. Use this when routing through an [LLM gateway or proxy](./llm-gateway.md "/docs/en/llm-gateway") that authenticates with bearer tokens rather than Anthropic API keys.
...
4. [`apiKeyHelper`](./settings.md#available-settings "/docs/en/settings#available-settings") script output. Use this for dynamic or rotating credentials, such as short-lived tokens fetched from a vault.
...
[Claude Code on the Web](./claude-code-on-the-web.md "/docs/en/claude-code-on-the-web") always uses your subscription credentials. `ANTHROPIC_API_KEY` and `ANTHROPIC_AUTH_TOKEN` in the sandbox environment do not override them.
...
This token authenticates with your Claude subscription and requires a Pro, Max, Team, or Enterprise plan. It is scoped to inference only and cannot establish [Remote Control](./remote-control.md "/docs/en/remote-control") sessions.
[Bare mode](./headless.md#start-faster-with-bare-mode "/docs/en/headless#start-faster-with-bare-mode") does not read `CLAUDE_CODE_OAUTH_TOKEN`. If your script passes `--bare`, authenticate with `ANTHROPIC_API_KEY` or an `apiKeyHelper` instead.