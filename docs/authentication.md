...
If you’re having trouble logging in, see [authentication troubleshooting](./overview.md#login-and-authentication).
...
* **Storage location**:
  ...
  + Claude Code manages `.credentials.json` through `/login` and `/logout`. To route requests through a custom API endpoint, set the [`ANTHROPIC_BASE_URL`](./overview.md) environment variable instead.
