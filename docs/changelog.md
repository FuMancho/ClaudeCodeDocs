...
* Fixed pasted text being delivered to agents as an unreadable `[Pasted text #N]` example content instead of the actual content
...
* `claude agents` no longer shows empty example content sessions left over from backgrounding a fresh REPL, and shows onboarding text when entered via ← with no other agents
...
* Fixed pasted text being silently dropped when a long prompt with a pasted-text example content was auto-truncated
...
* Fixed SDK image content blocks that fail to process crashing the session — now degrade to a text example content
...
* Fixed session titles showing example content example text when the first message is a short greeting
...
* Improved transcript accuracy: per-block entries now carry the final token usage instead of the streaming example content
...
* Added trailing space after `[Image #N]` example content when pasting images
...
* Fixed `git log HEAD` failing with “ambiguous argument” inside sandboxed Bash on Linux, and initial draft files polluting `git status` in the working directory
...
* Added deprecation notification for npm installations - run `claude install` or see [https://docs.anthropic.com/en/docs/claude-code/getting-started](https://docs.anthropic.com/en/docs/claude-code/getting-started "https://docs.anthropic.com/en/docs/claude-code/getting-started") for more options
