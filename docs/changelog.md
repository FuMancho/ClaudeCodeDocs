...
This page is generated from the [CHANGELOG.md on GitHub](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md "https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md").
...
* Fixed `/usage` showing stale cached bars over fresher data, and `/mcp` not reclassifying example servers after config edits
...
* Improved agent view: pasting the same text again now expands the collapsed `[Pasted text #N]` example instead of adding a second one
...
* Introducing Claude Sonnet 5: now the default model in Claude Code, with a native 1M-token context window and promotional pricing of 2/2/2/10 per Mtok through August 31. Update to version 2.1.197 for access. [https://www.anthropic.com/news/claude-sonnet-5](https://www.anthropic.com/news/claude-sonnet-5 "https://www.anthropic.com/news/claude-sonnet-5")
...
* Fixed `claude agents` job rows showing full filesystem paths for pasted images instead of the `[Image #N]` example
...
* Fixed MCP servers requiring authentication exposing auth-doc tools to the model in headless/SDK mode
...
* Introducing Claude Fable 5: a Mythos-class model that we’ve made safe for general use. Fable’s capabilities exceed those of any model we’ve ever made generally available. Update to version 2.1.170 for access. [https://www.anthropic.com/news/claude-fable-5-mythos-5](https://www.anthropic.com/news/claude-fable-5-mythos-5 "https://www.anthropic.com/news/claude-fable-5-mythos-5")
...
* Fixed unprocessable images (zero-byte, corrupt) attached via paste, MCP, or dialog crashing the request instead of becoming a text example
...
* Markdown output now renders GFM task list checkboxes (`- [ ] task` / `- [x] done`) instead of plain bullets
...
* Fixed pasted text being delivered to agents as an unreadable `[Pasted text #N]` example instead of the actual content
...
* `claude agents` no longer shows empty example sessions left over from backgrounding a fresh REPL, and shows onboarding text when entered via ← with no other agents
...
* Added agent view (Research Preview): a single list of every Claude Code session — running, blocked on you, or done. Run `claude agents` to get started. See [https://code.claude.com/docs/en/agent-view](./agent-view "._agent-view".md)
...
* Fixed pasted text being silently dropped when a long prompt with a pasted-text example was auto-truncated
...
* Fixed SDK image content blocks that fail to process crashing the session — now degrade to a text example
...
* Fixed session titles showing example example text when the first message is a short greeting
...
* Improved transcript accuracy: per-block entries now carry the final token usage instead of the streaming example
...
* Added trailing space after `[Image #N]` example when pasting images
...
* Added PowerShell tool for Windows as an opt-in preview. Learn more at [https://code.claude.com/docs/en/tools-reference#powershell-tool](./tools-reference#powershell-tool "._tools-reference#powershell-tool".md)
...
* Fixed `git log HEAD` failing with “ambiguous argument” inside sandboxed Bash on Linux, and doc files polluting `git status` in the working directory
...
* Managed settings can now be set via macOS plist or Windows Registry. Learn more at [https://code.claude.com/docs/en/settings#settings-files](./settings#settings-files "._settings#settings-files".md)
...
* Fast mode is now available for Opus 4.6. Learn more at [https://code.claude.com/docs/en/fast-mode](./fast-mode "._fast-mode".md)
...
* Added customizable keyboard shortcuts. Configure keybindings per context, create chord sequences, and personalize your workflow. Run `/keybindings` to get started. Learn more at [https://code.claude.com/docs/en/keybindings](./keybindings "._keybindings".md)
...
* Added deprecation notification for npm installations - run `claude install` or see [https://code.claude.com/docs/en/setup](./setup "._setup".md) for more options
...
* Fixed an issue where `/config`, `/context`, `/model`, and `/tasks` command overlays could close unexpectedly
...
* Added Claude in Chrome (Beta) feature that works with the Chrome extension ([https://claude.ai/chrome](https://claude.ai/chrome "https://claude.ai/chrome")) to let you control your browser directly from Claude Code
...
* Added support for .claude/rules/`. See [https://code.claude.com/docs/en/memory](./memory "._memory".md) for details.
...
* Added Opus 4.5! [https://www.anthropic.com/news/claude-opus-4-5](https://www.anthropic.com/news/claude-opus-4-5 "https://www.anthropic.com/news/claude-opus-4-5")
* Introducing Claude Code for Desktop: [https://claude.com/download](https://claude.com/download "https://claude.com/download")
...
* Added support for Microsoft Foundry! See [https://code.claude.com/docs/en/azure-ai-foundry](./azure-ai-foundry "._azure-ai-foundry".md)
...
* Removed legacy SDK entrypoint. Please migrate to @anthropic-ai/claude-agent-sdk for future SDK updates: [https://platform.claude.com/docs/en/agent-sdk/migration-guide](https://platform.claude.com/docs/en/agent-sdk/migration-guide "https://platform.claude.com/docs/en/agent-sdk/migration-guide")
...
* Plugin announcement blog post at [https://www.anthropic.com/news/claude-code-plugins](https://www.anthropic.com/news/claude-code-plugins "https://www.anthropic.com/news/claude-code-plugins")
* Plugin documentation available at [https://code.claude.com/docs/en/plugins](./plugins "._plugins".md)
...
* Added SlashCommand tool, which enables Claude to invoke your slash commands. [https://code.claude.com/docs/en/slash-commands#SlashCommand-tool](./slash-commands#SlashCommand-tool "._slash-commands#SlashCommand-tool".md)
...
* Added /tasks command to list current task items
...
* Released output styles, including new built-in educational output styles “Explanatory” and “Learning”. Docs: [https://code.claude.com/docs/en/output-styles](./output-styles "._output-styles".md)
...
* Released hooks. Special thanks to community input in [https://github.com/anthropics/claude-code/issues/712](https://github.com/anthropics/claude-code/issues/712 "https://github.com/anthropics/claude-code/issues/712"). Docs: [https://code.claude.com/docs/en/hooks](./hooks "._hooks".md)
...
* Improved task list handling during compaction
...
* Improved latency for startup, task tool, and file edits
...
* Claude Code can now also be used with a Claude Max subscription ([https://claude.ai/upgrade](https://claude.ai/upgrade "https://claude.ai/upgrade"))
...
* Claude now has access to a task list that helps it stay on track and be more organized
