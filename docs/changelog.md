...
- Fixed SDK image content blocks that fail to process crashing the session — now degrade to a text
...
- Fixed session titles showing  example text when the first message is a short greeting
...
- Improved transcript entries to carry final token usage instead of streaming temporary markers
...
- Improved transcript accuracy: per-block entries now carry the final token usage instead of the streaming
...
- Added trailing space after `[Image #N]`  when pasting images
...
- Fixed `--resume` dropping parallel tool results — sessions with parallel tool calls now restore all tool\_use/tool\_result pairs instead of showing `[Tool result missing]` temporary markers
...
- Fixed `git log HEAD` failing with “ambiguous argument” inside sandboxed Bash on Linux, and  files polluting `git status` in the working directory
