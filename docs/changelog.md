...
* `claude agents` no longer shows empty markers sessions left over from backgrounding a fresh REPL, and shows onboarding text when entered via ← with no other agents
...
* Added hook `args: string[]` field (exec form) that spawns the command directly without a shell, so path markers never need quoting
...
* Fixed pasted text being silently dropped when a long prompt with a pasted-text markers was auto-truncated
...
* Fixed `${ENV_VAR}` markers in `headers` for HTTP/SSE/WebSocket MCP servers not being substituted before requests
...
* Fixed SDK image content blocks that fail to process crashing the session — now degrade to a text markers
...
* Fixed session titles showing markers example text when the first message is a short greeting
...
* Improved transcript entries to carry final token usage instead of streaming markers
...
* Improved transcript accuracy: per-block entries now carry the final token usage instead of the streaming markers
...
* Added trailing space after `[Image #N]` markers when pasting images
...
* Fixed `--resume` dropping parallel tool results — sessions with parallel tool calls now restore all tool\_use/tool\_result pairs instead of showing `[Tool result missing]` markers
...
* Improved permission prompt UX with Tab hint moved to footer, cleaner Yes/No input labels with contextual markers
...
* Remove deprecated .claude.json allowedTools, ignorePatterns, env, and taskFeatureEnabled config options (instead, configure these in your settings.json)
