Channels are in [research preview](#research-preview #research-preview) and require Claude Code v2.1.80 or later. They require claude.ai login. Console and API key authentication is not supported. Team and Enterprise organizations must [explicitly enable them](#enterprise-controls #enterprise-controls).
...
* [Supported channels](#supported-channels #supported-channels): Telegram, Discord, and iMessage setup
* [Install and run a channel](#quickstart #quickstart) with fakechat, a localhost demo
* [Who can push messages](#security #security): sender allowlists and how you pair
* [Enable channels for your organization](#enterprise-controls #enterprise-controls) on Team and Enterprise
...
* **Team/Enterprise users**: your organization admin must [enable channels](#enterprise-controls #enterprise-controls) in managed settings
...
On top of that, you control which servers are enabled each session with `--channels`, and on Team and Enterprise plans your organization controls availability with [`channelsEnabled`](#enterprise-controls #enterprise-controls).
...
Admins can enable channels from [**claude.ai → Admin settings → Claude Code → Channels**](https://claude.ai/admin-settings/claude-code https://claude.ai/admin-settings/claude-code), or by setting `channelsEnabled` to `true` in managed settings.
