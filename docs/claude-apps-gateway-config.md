...
To write your first one, start from the [quickstart](./claude-apps-gateway#quickstart "._claude-apps-gateway#quickstart".md), which builds a minimal working config and runs it. Once you have a config you’re happy with, the [deployment guide](./claude-apps-gateway-deploy "._claude-apps-gateway-deploy".md) covers containerizing and hosting it on Kubernetes, Cloud Run, or your own platform.
...
The [complete example](#complete-example "#complete-example") at the end of this page exercises every section.

## [​](#file-structure "#file-structure") File structure

Five sections are [required](#required-sections "#required-sections"). Every other section is [optional](#optional-sections "#optional-sections"), and an omitted section takes its defaults. Unknown keys fail boot, so a typo surfaces as a named error rather than a silently ignored setting.
**Required sections:**

* [`listen`](#listen "#listen"): bind address, public URL, TLS termination
* [`oidc`](#oidc "#oidc"): your identity provider (IdP), including issuer, client, claim mapping, and who may sign in
* [`session`](#session "#session"): the bearer tokens the gateway mints, with secret and lifetime
* [`store`](#store "#store"): PostgreSQL, for device grants and rate-limit counters
* [`upstreams`](#upstreams "#upstreams"): where inference goes, whether Anthropic, Amazon Bedrock, Claude Platform on AWS, Google Cloud’s Agent Platform, or Microsoft Foundry

**Optional sections:**

* [`admin`](#admin "#admin"): Admin API auth and retention for spend limits
* [`enforcement`](#enforcement "#enforcement"): spend-limit fail-open or fail-closed behavior
* [`models`](#models "#models") and `auto_include_builtin_models`: admin-curated model list and per-upstream IDs
* [`managed`](#managed "#managed"): managed settings policies by IdP group
* [`telemetry`](#telemetry "#telemetry"): OTLP forwarding to your observability stack
* [`access_control`, `limits`, `timeouts`, `rate_limits`](#http-tuning "#http-tuning"): IP allow/deny, request size caps, upstream time-to-first-byte, and per-IP sign-in limits

## [​](#secret-expansion "#secret-expansion") Secret expansion
...
## [​](#required-sections "#required-sections") Required sections
...
OpenID Connect (OIDC) is the SSO protocol the gateway uses with your identity provider; see [Identity provider setup](./claude-apps-gateway-deploy#identity-provider-setup "._claude-apps-gateway-deploy#identity-provider-setup".md) for what to register on the IdP side.


| Field | Required | Description |
| --- | --- | --- |
| `issuer` | Yes | OIDC discovery base. Must serve discovery at `/.well-known/openid-configuration`. Use HTTPS in production; the gateway accepts an `http://` issuer. A loopback issuer such as `http://localhost:8081` is rejected by the [SSRF guard](./claude-apps-gateway-deploy#threat-model-summary "._claude-apps-gateway-deploy#threat-model-summary".md) unless `CLAUDE_GATEWAY_ALLOW_LOOPBACK=1` is set in the gateway’s environment. |
...
| `scopes` | No | Full override of the OIDC scopes the gateway requests. Default `[openid, profile, email, offline_access]`. Set when your IdP rejects scopes it doesn’t recognize, or requires a custom scope to emit groups or email. Must include `openid`. Dropping `offline_access` disables refresh tokens, so developers re-run the browser login every `session.ttl_hours`. See [Identity provider setup](./claude-apps-gateway-deploy#identity-provider-setup "._claude-apps-gateway-deploy#identity-provider-setup".md) for per-IdP scope recipes such as Google’s refresh-token flow. |
...
| Field | Required | Description |
| --- | --- | --- |
| `postgres_url` | Yes | `postgres://` or `postgresql://` URL. Required: the device-grant rendezvous, where the browser callback writes and the polling CLI reads, needs cross-replica state. The gateway runs its own schema migrations at boot, so the role needs `CREATE TABLE` on the target schema. If your security policy prohibits DDL from the application role, run the migrations with an admin role, initially and again whenever a new release ships migrations, and grant the app role `SELECT, INSERT, UPDATE, DELETE` on the gateway’s tables. See [Upgrades](./claude-apps-gateway-deploy#upgrades "._claude-apps-gateway-deploy#upgrades".md) and [Postgres](./claude-apps-gateway-deploy#postgres "._claude-apps-gateway-deploy#postgres".md). |
...
| `max_connections` | No | Postgres connection-pool size per replica. Default `5`, which is conservative and friendly to shared databases. With [spend limits](#admin "#admin") enabled, the hot path does a few operations per inference request, so raise it for a dedicated database under load, and keep replicas × this below the database’s `max_connections`. |
...
For the client-side Amazon Bedrock deployment that the gateway replaces or fronts, see [Claude Code on Amazon Bedrock](./amazon-bedrock "._amazon-bedrock".md). The gateway-side upstream:
...
| Region | `region:` is the API endpoint region. Cross-region inference profiles route across the geo (US, EU, APAC) regardless of which one you pick. For non-US regions or provisioned-throughput ARNs, add a [`models:`](#models "#models") block with the right per-upstream IDs. |
...
For the client-side deployment of the same platform, see [Claude Code on Claude Platform on AWS](./claude-platform-on-aws "._claude-platform-on-aws".md). The gateway-side upstream:
...
The platform runs in a separate AWS account from Amazon Bedrock and signs SigV4 requests for its own service name, `aws-external-anthropic`, so a Bedrock-scoped IAM role doesn’t authorize it. An API key in `auth.api_key` takes precedence when SigV4 credentials are also set. An empty `auth` block uses the AWS SDK’s default credential chain, the same chain the [Amazon Bedrock](#amazon-bedrock "#amazon-bedrock") upstream uses.
...
Because the platform resolves first-party model IDs, the built-in catalog routes to it with no [`models:`](#models "#models") block. When you curate a `models:` list, key the entry `anthropicAws:` with the first-party ID.

#### [​](#google-cloud-agent-platform "#google-cloud-agent-platform") Google Cloud Agent Platform

For the equivalent client-side setup, see [Claude Code on Google Cloud](./google-vertex-ai "._google-vertex-ai".md). The gateway-side upstream:
...
Set `region: global` to use the [global endpoint for Google Cloud’s Agent Platform](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/locations "https://cloud.google.com/vertex-ai/generative-ai/docs/learn/locations") instead of a regional one. Google then routes each request to an available region, so you don’t track per-region model availability. Setting a specific region pins every request to it.
...
For the client-side Microsoft Foundry deployment, see [Claude Code on Microsoft Foundry](./microsoft-foundry "._microsoft-foundry".md). The gateway-side upstream:
...
| Deployments | Microsoft Foundry uses admin-chosen deployment names, not canonical model IDs. Add a [`models:`](#models "#models") block mapping each canonical ID to your deployment name. |
...
#### [​](#multiple-upstreams "#multiple-upstreams") Multiple upstreams
...
| Lever | How |
| --- | --- |
| Different regions | One Amazon Bedrock upstream per region, each with its own `region:`. With [`auto_include_builtin_models: true`](#models "#models") the cross-region inference profiles route automatically; for region-pinned deployments use a `models:` block. |
...
Optional. Enables `/v1/organizations/spend_limits`, which mirrors Anthropic’s public Admin API, and per-developer spend enforcement on `/v1/messages`. See [Spend limits](./claude-apps-gateway-spend-limits "._claude-apps-gateway-spend-limits".md) for how caps are set and enforced; this section covers the `gateway.yaml` keys that turn the feature on and tune it.
...
| `read_keys` | No | Array of `{id, key}`. Read-only: every `GET` endpoint, including listing caps, fetching one by ID, and reading [`/effective`](./claude-apps-gateway-spend-limits#%2Feffective "._claude-apps-gateway-spend-limits#%2Feffective".md) and [`/audit`](./claude-apps-gateway-spend-limits#%2Faudit "._claude-apps-gateway-spend-limits#%2Faudit".md). |
...
| Field | Required | Description |
| --- | --- | --- |
| `fail_closed_on_error` | No | Default `false`. Spend enforcement fails open on a Postgres outage, so inference stays up. Set `true` to fail closed: over-cap developers are blocked, but so is everyone else if the store is unreachable. Has no effect without an [`admin:`](#admin "#admin") block. |
...
#### [​](#what-goes-in-cli "#what-goes-in-cli") What goes in `cli`
...
The full key reference is in [Claude Code settings](./settings#available-settings "._settings#available-settings".md). The keys most operators reach for first:
...
| `permissions.allow` / `.deny` | CLI | Tool and command rules. See [Permissions](./permissions "._permissions".md). |
| `permissions.disableBypassPermissionsMode` | CLI | Set to `disable` to block [`bypassPermissions`](./permission-modes#skip-all-checks-with-bypasspermissions-mode "._permission-modes#skip-all-checks-with-bypasspermissions-mode".md), the mode that skips permission prompts, and the `--dangerously-skip-permissions` flag |
...
| `hooks` | CLI | Org-wide [hooks](./hooks "._hooks".md) |
...
The gateway’s [telemetry](#telemetry "#telemetry") configuration pushes `OTEL_EXPORTER_OTLP_ENDPOINT`, so setting `telemetry.forward_to` triggers the dialog on each interactive client. The dialog protects the developer’s machine from a compromised or hostile gateway, not the organization from the developer.
...
#### [​](#precedence-with-other-managed-sources "#precedence-with-other-managed-sources") Precedence with other managed sources
...
1. The [policy helper](./settings#compute-managed-settings-with-a-policy-helper "._settings#compute-managed-settings-with-a-policy-helper".md)
...
* On machines with an admin-deployed managed source, it is ignored unless the highest-priority source opts in with [`parentSettingsBehavior: "merge"`](./settings#available-settings "._settings#available-settings".md).
* It is never merged while a [`policyHelper`](./settings#compute-managed-settings-with-a-policy-helper "._settings#compute-managed-settings-with-a-policy-helper".md) is configured.
* When merged, it passes through a restrictive-only allowlist. [Restrict parent settings](./claude-apps-gateway#restrict-parent-settings "._claude-apps-gateway#restrict-parent-settings".md) lists which allow-direction settings still apply without the `allowManaged*Only` locks.

The following keys are honored when any admin source above the user-writable HKCU tier sets them, regardless of which source provides the rest of the policy. When a [`policyHelper`](./settings#compute-managed-settings-with-a-policy-helper "._settings#compute-managed-settings-with-a-policy-helper".md) is configured, its output is the only source these checks read:
...
* [`allowAllClaudeAiMcps`](./settings#available-settings "._settings#available-settings".md): allow-only override for the claude.ai MCP server allowlist
* `sandbox.bwrapPath` and `sandbox.socatPath`: filesystem paths to the [sandbox](./sandboxing "._sandboxing".md) helper binaries
* [`forceRemoteSettingsRefresh`](./server-managed-settings "._server-managed-settings".md): blocks startup until remote managed settings are freshly fetched, so an MDM or file policy that sets it is honored even when a cached remote payload that lacks the key is the highest-priority source

Every other key, including `disableBypassPermissionsMode`, comes from the highest-priority source only. Two [parent-settings](./claude-apps-gateway#restrict-parent-settings "._claude-apps-gateway#restrict-parent-settings".md) checks read every admin source:
...
See [Settings precedence](./settings#settings-precedence "._settings#settings-precedence".md) for the same rules on the settings page.
...
The CLI sends OpenTelemetry Protocol (OTLP) over HTTP metrics, logs, and, when enabled, traces to the gateway, which relays them verbatim to each configured destination. See [Monitoring usage](./monitoring-usage "._monitoring-usage".md) for the metrics and events the CLI emits.
...
[Traces](./monitoring-usage#traces-beta "._monitoring-usage#traces-beta".md) additionally require `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` on each client. The gateway doesn’t push that variable, so set it through a managed policy’s `env` block. It isn’t on the CLI’s safe list, so delivering it through a policy is covered by the same [security approval dialog](#managed "#managed") that the pushed OTLP endpoint already triggers.
Both protobuf and JSON OTLP encodings are relayed, and any OpenTelemetry-compatible backend works as a destination.

### [​](#http-tuning "#http-tuning") HTTP tuning
...
| `rate_limits` | `device_authorization.max` / `.window_seconds` | 30 / 600 | Per-IP rate limit on the unauthenticated device-authorization endpoint. Raise for a large org behind a shared egress IP or NAT. These limits apply only to the device-grant sign-in flow, not to `/v1/messages` inference. See [User-code brute-force resistance](./claude-apps-gateway-deploy#user-code-brute-force-resistance "._claude-apps-gateway-deploy#user-code-brute-force-resistance".md). |
...
This full reference config exercises every core section; the [HTTP tuning blocks](#http-tuning "#http-tuning") keep their defaults. Copy it, delete what you don’t need, and fill in your values. The config in the [Quickstart](./claude-apps-gateway#quickstart "._claude-apps-gateway#quickstart".md) is a minimal version of this.
...
## [​](#client-side-managed-settings "#client-side-managed-settings") Client-side managed settings

Everything above configures the gateway server. Pointing developer machines at it is configured separately, on each device, through Claude Code’s [managed settings](./settings#settings-files "._settings#settings-files".md). The gateway can’t push the login keys itself, because they’re what tell the client where the gateway is.
...
`parentSettingsBehavior: "merge"` keeps Claude Desktop’s policy delivery to its embedded Claude Code sessions working; [Deliver policy to Claude Desktop sessions](./claude-apps-gateway#deliver-policy-to-claude-desktop-sessions "._claude-apps-gateway#deliver-policy-to-claude-desktop-sessions".md) explains the mechanism and where the opt-in must sit.
...
A registry policy on Windows or a managed-preferences plist on macOS replaces the `managed-settings.json` file rather than merging with it, apart from the [exception keys and cross-source checks above](#precedence-with-other-managed-sources "#precedence-with-other-managed-sources"). All three keys in this snippet follow the highest-priority-source rule, so fleets that deliver policy through Group Policy or configuration profiles must put all three in that mechanism instead.
...
* [Claude apps gateway overview](./claude-apps-gateway "._claude-apps-gateway".md): quickstart and developer connection
* [Deployment guide](./claude-apps-gateway-deploy "._claude-apps-gateway-deploy".md): IdP setup, container image, Kubernetes and Cloud Run, and operations
* [Spend limits](./claude-apps-gateway-spend-limits "._claude-apps-gateway-spend-limits".md): per-developer caps and the Admin API