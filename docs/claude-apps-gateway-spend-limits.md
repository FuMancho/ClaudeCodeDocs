...
| --- | --- | --- |
| `scope.type` | `user`, `rbac_group`, `organization` | `user` targets one developer by their OpenID Connect (OIDC) `sub`, the stable user ID your identity provider assigns; pass it as `scope.user_id`. `rbac_group` targets an [IdP group](./claude-apps-gateway-config#managed "._claude-apps-gateway-config#managed".md) by name; pass it as `scope.rbac_group_id`. `organization` is the org-wide default. The gateway accepts all three; Anthropic’s public `POST` is user-only today. |
...
A group or organization cap is a per-seat default that each member inherits, not a shared pool. Per period, a developer’s effective cap resolves in this order: a per-user override, then the most restrictive of their group caps, then the org default, then unlimited. [`admin.group_limit_mode: max`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md) flips the multi-group tie-break to least-restrictive instead.

### [​](#authenticate-to-the-admin-api "#authenticate-to-the-admin-api") Authenticate to the admin API

Send one of:

* An `x-api-key` header matching a key in [`admin.write_keys`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md) for full access, or `admin.read_keys` for `GET`-only access. Each key carries an `id` that appears in the audit log as `admin-key:<id>`, so give Terraform, CI, and each automation its own.
* A gateway bearer token whose `groups` claim includes one of [`admin.admin_groups`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md). This is full access and audits as `oidc:<sub>`, so prefer it for human admins.

## [​](#how-enforcement-works "#how-enforcement-works") How enforcement works

On each `/v1/messages` request, the gateway resolves the developer’s caps and period-to-date spend in one Postgres query. If they’re over any cap, the request returns `429` with `error.type: billing_error` and the header `x-should-retry: false`. The message is `spend limit reached`, followed by your [`admin.blocked_message`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md) if set.
...
### [​](#postgres-availability "#postgres-availability") Postgres availability

The pre-check queries Postgres with a two-second timeout. If the store is unreachable or times out, enforcement fails open by default: the request proceeds and the gateway logs a warning. Set [`enforcement.fail_closed_on_error: true`](./claude-apps-gateway-config#enforcement "._claude-apps-gateway-config#enforcement".md) to fail closed instead, which returns the same `429 billing_error` with the message `spend limit unavailable`. Fail-open keeps a store outage from becoming an inference outage; fail-closed guarantees no unmetered spend.

## [​](#admin-api-reference "#admin-api-reference") Admin API reference
...
## [​](#data-lifecycle "#data-lifecycle") Data lifecycle
...
| --- | --- | --- |
| `spend` | Per-principal period-to-date counters in cents | [`admin.spend_retention_months`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md), default 13 |
| `spend_limits` | The configured caps | Until deleted via the API |
| `admin_audit` | The mutation trail | [`admin.audit_retention_days`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md), default 365 |
| `principal_emails` | Each principal’s last-seen email, display name, and IdP groups. Contains PII. | [`admin.identity_retention_days`](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md) since last activity, default 90 |
...
* [`admin` and `enforcement` configuration](./claude-apps-gateway-config#admin "._claude-apps-gateway-config#admin".md): enabling the admin API and tuning retention
* [Deployment guide](./claude-apps-gateway-deploy#postgres "._claude-apps-gateway-deploy#postgres".md): Postgres schema and backup guidance