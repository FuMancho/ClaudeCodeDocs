...
## [​](#enable-claude-code-features-with-settingsources #enable-claude-code-features-with-settingsources) Enable Claude Code features with settingSources

The setting sources option ([`setting_sources`](./python.md#claude-agent-options) in Python, [`settingSources`](./typescript.md#setting-source) in TypeScript) controls which filesystem-based settings the SDK loads. Without it, your agent won’t discover skills, `CLAUDE.md` files, or project-level hooks.
...
Each source loads settings from a specific location, where `<cwd>` is the working directory you pass via the `cwd` option (or the process’s current directory if unset). For the full type definition, see [`SettingSource`](./typescript.md#setting-source) (TypeScript) or [`SettingSource`](./python.md#setting-source) (Python).
...
For more on creating and using skills, see [Agent Skills in the SDK](./skills.md).
...
Hook callbacks receive the tool input and return a decision dict. Returning `{}` (an empty dict) means allow the tool to proceed. Returning `{"decision": "block", "reason": "..."}` prevents execution and the reason is sent to Claude as the tool result. See the [hooks guide](./hooks.md) for the full callback signature and return types.
...
| Give the agent reference material it loads when relevant | [Skills](./skills.md) | `settingSources` + `allowedTools: ["Skill"]` |
| Run a reusable workflow (deploy, review, release) | [User-invocable skills](./skills.md) | `settingSources` + `allowedTools: ["Skill"]` |
