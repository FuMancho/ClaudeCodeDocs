...
As of v2.1.179, commits that Claude creates in a web session include a `Claude-Session: <url>` git trailer, and PR bodies include the session URL on its own line. From v2.1.182, set [`attribution.sessionUrl`](./settings.md#attribution-settings) to `false` to omit the trailer and the PR-body link.
