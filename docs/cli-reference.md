...
| `--model` | Sets the model for the current session with an alias for the latest model (`sonnet`, `opus`, `haiku`, or `fable`) or a model’s full name. Overrides the [`model`](./settings.md#available-settings) setting and [`ANTHROPIC_MODEL`](./model-config.md#environment-variables) | `claude --model claude-sonnet-4-6` |
...
| `--teammate-mode` | Set how [agent team](./agent-teams.md) teammates display: `in-process` (default), `auto`, or `tmux`. The default changed from `auto` in v2.1.179. Overrides the [`teammateMode`](./settings.md#available-settings) setting for this session. See [Choose a display mode](./agent-teams.md#choose-a-display-mode) | `claude --teammate-mode auto` |
