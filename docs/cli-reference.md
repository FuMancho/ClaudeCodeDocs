...
| `--teammate-mode` | Set how [agent team](./agent-teams.md) teammates display: `auto` (default), `in-process`, or `tmux`. Overrides the [`teammateMode`](./settings.md#available-settings) setting for this session. See [Choose a display mode](./agent-teams.md#choose-a-display-mode) | `claude --teammate-mode in-process` |
