...
| --- | --- | --- |
| `--add-dir` | Add additional working directories for Claude to read and edit files. Grants file access; most `.claude/` configuration is [not discovered](./permissions.md#additional-directories-grant-file-access-not-configuration) from these directories. Validates each path exists as a directory | `claude --add-dir ../apps ../lib` |
...
| `--effort` | Set the [effort level](./model-config.md#adjust-effort-level) for the current session. Options: `low`, `medium`, `high`, `max` (Opus 4.6 only). Session-scoped and does not persist to settings | `claude --effort high` |
...
| `--enable-auto-mode` | Unlock [auto mode](./permission-modes.md#eliminate-prompts-with-auto-mode) in the `Shift+Tab` cycle. Requires a Team, Enterprise, or API plan and Claude Sonnet 4.6 or Opus 4.6 | `claude --enable-auto-mode` |
...
| `--teammate-mode` | Set how [agent team](./agent-teams.md) teammates display: `auto` (default), `in-process`, or `tmux`. See [Choose a display mode](./agent-teams.md#choose-a-display-mode) | `claude --teammate-mode in-process` |
