...
| `claude respawn <id>` | Restart a stopped [background session](./agent-view.md#manage-sessions-from-the-shell) with its conversation intact. Use `--all` to restart every stopped session | `claude respawn 7c5dcf5d` |
| `claude rm <id>` | Remove a [background session](./agent-view.md#manage-sessions-from-the-shell) from the list | `claude rm 7c5dcf5d` |
...
| `--json-schema` | Get validated JSON output matching a JSON Schema after agent completes its workflow (print mode only, see [structured outputs](./structured-outputs.md)) | `claude -p --json-schema '{"type":"object","properties":{...}}' "query"` |
...
These flags apply only to the current invocation. For persistent personas you can switch between and share across a project, use [output styles](./output-styles.md). For project conventions Claude should always follow, use [CLAUDE.md](./memory.md). The [Agent SDK guide on system prompts](./modifying-system-prompts.md#decide-on-a-starting-point) covers the same decision in more depth.
