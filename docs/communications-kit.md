...
Treat everything here as draft copy, not finished copy. Rewrite each message in your organization’s voice, swap the example tasks for real bugs and modules from your own codebase, and replace the `[bracketed s]` before sending. The announcements that drive adoption are the ones that read like someone at your company wrote them.
...
Copy the message body from each block directly into Slack or Teams. Replace `[bracketed s]` before sending.
