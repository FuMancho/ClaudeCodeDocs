...
This page covers how to [track your costs](#track-your-costs "#track-your-costs"), [manage costs for your organization](#manage-costs-for-your-organization "#manage-costs-for-your-organization"), and [reduce token usage](#reduce-token-usage "#reduce-token-usage").

## [​](#track-your-costs "#track-your-costs") Track your costs
...
In the [VS Code extension](./vs-code#check-account-and-usage "._vs-code#check-account-and-usage".md), the same breakdown appears in the Account & usage dialog with a Day and Week toggle. Requires Claude Code v2.1.174 or later.

### [​](#set-a-spend-limit-on-pro-and-max "#set-a-spend-limit-on-pro-and-max") Set a spend limit on Pro and Max
...
## [​](#manage-costs-for-your-organization "#manage-costs-for-your-organization") Manage costs for your organization
...
| --- | --- | --- | --- |
| [Claude for Teams or Enterprise](#claude-for-teams-and-enterprise "#claude-for-teams-and-enterprise") | [Spend report in org analytics](https://support.claude.com/en/articles/12883420-view-usage-analytics-for-team-and-enterprise-plans "https://support.claude.com/en/articles/12883420-view-usage-analytics-for-team-and-enterprise-plans") | Spend limits in admin settings | [Spend report CSV](https://support.claude.com/en/articles/12883420-view-usage-analytics-for-team-and-enterprise-plans "https://support.claude.com/en/articles/12883420-view-usage-analytics-for-team-and-enterprise-plans"); [Enterprise Analytics API](https://platform.claude.com/docs/en/api/admin/analytics "https://platform.claude.com/docs/en/api/admin/analytics") on Enterprise |
| [Claude Console (API)](#claude-console "#claude-console") | [Console usage page](https://platform.claude.com/usage "https://platform.claude.com/usage") | Workspace spend limits | [Console dashboard](https://platform.claude.com/claude-code "https://platform.claude.com/claude-code"), [Claude Code Analytics API](https://platform.claude.com/docs/en/build-with-claude/claude-code-analytics-api "https://platform.claude.com/docs/en/build-with-claude/claude-code-analytics-api") |
| [Amazon Bedrock, Google Cloud’s Agent Platform, or Microsoft Foundry](#cloud-providers "#cloud-providers") | Your cloud billing console | Your cloud’s budget controls | [OpenTelemetry](./monitoring-usage "._monitoring-usage".md) or an [LLM gateway](./llm-gateway "._llm-gateway".md) |

[OpenTelemetry export](./monitoring-usage "._monitoring-usage".md) works on every setup and is the only option that streams per-user token and cost metrics into your own observability stack in near real time.

### [​](#claude-for-teams-and-enterprise "#claude-for-teams-and-enterprise") Claude for Teams and Enterprise
...
For per-user reporting, the [Console dashboard](https://platform.claude.com/claude-code "https://platform.claude.com/claude-code") shows spend and accepted lines per member, and the [Claude Code Analytics API](https://platform.claude.com/docs/en/build-with-claude/claude-code-analytics-api "https://platform.claude.com/docs/en/build-with-claude/claude-code-analytics-api") returns the same daily per-user metrics programmatically with an [Admin API key](https://platform.claude.com/settings/admin-keys "https://platform.claude.com/settings/admin-keys"). See [analytics for API customers](./analytics#access-analytics-for-api-customers "._analytics#access-analytics-for-api-customers".md).
...
On Amazon Bedrock, Google Cloud’s Agent Platform, and Microsoft Foundry, Claude Code is billed per token to your cloud account, and spend controls live in your cloud provider’s billing console. Claude Code does not send metrics from your cloud back to Anthropic, so the [analytics dashboards](./analytics "._analytics".md) and the Claude Code Analytics API do not cover this usage.
For per-user cost attribution, you have three options:

* **OpenTelemetry**: [export metrics](./monitoring-usage "._monitoring-usage".md) from each developer’s machine to your own observability stack. This gives you per-user token counts, costs, and tool activity regardless of provider.
* **A Claude apps gateway**: a self-hosted [Claude apps gateway](./claude-apps-gateway "._claude-apps-gateway".md) provides per-user usage attribution, OTLP metrics with token counts, and [per-user spend limits](./claude-apps-gateway-spend-limits "._claude-apps-gateway-spend-limits".md) on these providers.
* **An LLM gateway**: route all Claude Code traffic through a proxy that tracks spend per key. Several large enterprises reported using [LiteLLM](./llm-gateway "._llm-gateway".md), an open-source tool that tracks spend by key. This project is unaffiliated with Anthropic and has not been audited for security.

### [​](#when-a-developer-asks-about-a-limit "#when-a-developer-asks-about-a-limit") When a developer asks about a limit
...
## [​](#background-token-usage "#background-token-usage") Background token usage
...
## [​](#understanding-changes-in-claude-code-behavior "#understanding-changes-in-claude-code-behavior") Understanding changes in Claude Code behavior
...
* **Subscription plans** (Pro, Max, Team, Enterprise): sign in at [claude.ai](https://claude.ai "https://claude.ai"), click your initials in the lower left, and select **Get help**
* **Console (API) billing**: sign in at [platform.claude.com](https://platform.claude.com "https://platform.claude.com"), click your initials, and select **Get help**

See [How to get support](https://support.claude.com/en/articles/9015913-how-to-get-support "https://support.claude.com/en/articles/9015913-how-to-get-support") for the full flow, including who can reach a human agent on each plan.