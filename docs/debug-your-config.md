...
For installation, authentication, and connectivity problems, see [Troubleshoot installation and login](./troubleshoot-install.md "/docs/en/troubleshoot-install") instead.
...
If a memory file is missing from `/memory`, check its location against [how CLAUDE.md files load](./memory.md#how-claude-md-files-load "/docs/en/memory#how-claude-md-files-load"). Subdirectory `CLAUDE.md` files load on demand when Claude reads a file in that directory with the Read tool, not at session start.
...
Adherence drops when an instruction is vague enough to interpret multiple ways, when two files give conflicting direction, or when the file has grown long enough that individual rules get less attention. [Write effective instructions](./memory.md#write-effective-instructions "/docs/en/memory#write-effective-instructions") covers the specificity, size, and structure patterns that keep adherence high.

CLAUDE.md and permissions solve different problems. CLAUDE.md tells Claude how your project works so it makes good decisions. [Permissions](./permissions.md "/docs/en/permissions") and [hooks](./hooks.md "/docs/en/hooks") enforce limits regardless of what Claude decides. Use CLAUDE.md for “we do it this way here.” Use permissions or hooks for security boundaries and anything that must never happen, where you need a guarantee instead of guidance.

## [​](#check-resolved-settings "#check-resolved-settings") Check resolved settings

Settings merge across managed, user, project, and local scopes. Managed settings always win when present. Among the rest, the closer scope overrides the broader one in the order local, then project, then user. Some settings can also be set by command-line flags or [environment variables](./env-vars.md "/docs/en/env-vars"), which act as another override layer. When a setting doesn’t seem to apply, the value you set is usually being overridden by another scope or an environment variable.
...
Run `/status` to see which settings sources are active, including whether managed settings are in effect. To understand which scope wins for a given key, see [How scopes interact](./settings.md#how-scopes-interact "/docs/en/settings#how-scopes-interact").
...
For configuration locations and scope rules, see [MCP](./mcp.md "/docs/en/mcp").
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Hook never fires | `matcher` is a JSON array instead of a string | Use a single string with `|` to match multiple tools, for example `"Edit|Write"`. See [matcher patterns](./hooks.md#matcher-patterns "/docs/en/hooks#matcher-patterns"). |
...
| Hook never fires | Hooks are defined in a standalone file instead of `settings.json` | There is no standalone hooks file for project or user config. Define hooks under the `"hooks"` key in `settings.json`. Only [plugins](./plugins-reference.md#hooks "/docs/en/plugins-reference#hooks") load a separate `hooks/hooks.json`. See [hook configuration](./hooks.md "/docs/en/hooks"). |
...
| A `settings.json` value seems ignored | The same key is set in `settings.local.json` | `settings.local.json` overrides `settings.json`, and both override `~/.claude/settings.json`. See [settings precedence](./settings.md#how-scopes-interact "/docs/en/settings#how-scopes-interact"). |
...
| Subagent ignores `CLAUDE.md` instructions | The built-in Explore and Plan agents skip `CLAUDE.md`. Custom subagents load it the same way the main conversation does | For Explore or Plan, restate the instruction in your delegating prompt. For a custom subagent, put critical instructions in the agent file body, which becomes the agent’s system prompt. See [what loads at startup](./sub-agents.md#what-loads-at-startup "/docs/en/sub-agents#what-loads-at-startup"). |
| Cleanup logic never runs at session end | No `SessionEnd` hook configured | Add a `SessionEnd` hook in `settings.json`. See the [hook events list](./hooks.md#hook-events "/docs/en/hooks#hook-events"). |
| MCP servers in `.mcp.json` never load | File is under `.claude/` or uses Claude Desktop’s config format | Project MCP config goes at the repository root as `.mcp.json`, not inside `.claude/`. See [MCP configuration](./mcp.md "/docs/en/mcp"). |
| MCP servers added under `mcpServers` in `settings.json` never appear | `settings.json` does not read an `mcpServers` key | Define project servers in `.mcp.json` at the repository root, or run `claude mcp add --scope user` for user-scoped servers. See [MCP configuration](./mcp.md "/docs/en/mcp"). |
...
| `Bash(rm *)` deny rule doesn’t block `/bin/rm` or `find -delete` | Prefix rules match the literal command string, not the underlying executable | Add explicit patterns for each variant, or use a [PreToolUse hook](./hooks-guide.md "/docs/en/hooks-guide") or the [sandbox](./sandboxing.md "/docs/en/sandboxing") for a hard guarantee. |
...
* **[`.claude` directory reference](./claude-directory.md "/docs/en/claude-directory")**: every config file location and what reads it
* **[Settings](./settings.md "/docs/en/settings")**: precedence order and the full key list
* **[Hooks reference](./hooks.md "/docs/en/hooks")**: event names, payloads, and `--debug hooks` output format
* **[MCP](./mcp.md "/docs/en/mcp")**: server configuration, approval, and `/mcp` output
* **[Troubleshoot installation and login](./troubleshoot-install.md "/docs/en/troubleshoot-install")**: `command not found`, PATH, and authentication problems
* **[Troubleshooting](./troubleshooting.md "/docs/en/troubleshooting")**: performance, hangs, and search issues