...
If a memory file is missing from `/memory`, check its location against [how CLAUDE.md files load](./memory#how-claude-md-files-load "_memory#how-claude-md-files-load".md). Subdirectory `CLAUDE.md` files load on demand when Claude reads a file in that directory with the Read tool, not at session start.
...
If targeted checks don’t isolate the cause, or your configuration is in an unknown state, compare against a session that loads nothing from your usual setup. Point [`CLAUDE_CONFIG_DIR`](./env-vars "_env-vars".md) at an empty directory to bypass everything under `~/.claude`, and launch from a directory that has no `.claude` folder, `.mcp.json`, or `CLAUDE.md` so project configuration is also skipped.
