...
```
A `.deb` installed this way doesn’t receive updates. To get updates through apt, add the repository as shown above, or uncomment the `deb` line in the task entry the package writes to `/etc/apt/sources.list.d/claude-desktop.list`.
