...
* Windows 10 or 11 with [WSL 2](https://learn.microsoft.com/windows/wsl/install "https://learn.microsoft.com/windows/wsl/install"). WSL 1 isn’t supported.
* At least one installed distribution (for example, Ubuntu).
* `git` installed inside the distribution.

## [​](#start-a-wsl-session "#start-a-wsl-session") Start a WSL session
...
## [​](#what-works-in-a-wsl-session "#what-works-in-a-wsl-session") What works in a WSL session
...
## [​](#managed-devices "#managed-devices") Managed devices

On devices managed by an organization, WSL sessions may be unavailable. If session start fails with a message that the device is managed, that’s controlled by your administrator. Administrators: see [how settings reach devices](./admin-setup#decide-how-settings-reach-devices "._admin-setup#decide-how-settings-reach-devices".md) in the deployment guide.