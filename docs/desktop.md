...
For [scheduled recurring work](./overview.md), [keyboard shortcuts](#keyboard-shortcuts), or [sending tasks from your phone](#sessions-from-dispatch), see the linked pages and sections. If you already use the terminal-based CLI, see the [CLI comparison](#coming-from-the-cli) for what carries over.
...
The `dontAsk` permission mode is available only in the [CLI](./overview.md#allow-only-pre-approved-tools-with-dontask-mode).
...
Click **+ New session** in the sidebar, or press **Cmd+N** on macOS or **Ctrl+N** on Windows, to work on multiple tasks in parallel. Press **Ctrl+Tab** and **Ctrl+Shift+Tab** to cycle through sessions in the sidebar. For Git repositories, each session gets its own isolated copy of your project using [Git worktrees](./overview.md), so changes in one session don’t affect other sessions until you commit them.
...
To include gitignored files like `.env` in new worktrees, create a [`.worktreeinclude` file](./overview.md#copy-gitignored-files-into-worktrees) in your project root.
...
Dispatch is one of several ways to work with Claude when you’re away from your terminal. See [Platforms and integrations](./overview.md#work-when-you-are-away-from-your-terminal) to compare it with Remote Control, Channels, Slack, and scheduled tasks.
...
For local and [SSH](#ssh-sessions) sessions, click the **+** button next to the prompt box and select **Connectors** to add integrations like Google Calendar, Slack, GitHub, Linear, Notion, and more. You can add connectors before or during a session. The **+** button is not available in remote sessions, but [routines](./overview.md) configure connectors at routine creation time.
...
| `disableAutoMode` | set to `"disable"` to prevent users from enabling [Auto](./overview.md#eliminate-prompts-with-auto-mode) mode. Removes Auto from the mode selector. Also accepted under `permissions`. |
| `autoMode` | customize what the auto mode classifier trusts and blocks across your organization. See [Configure auto mode](./overview.md). |
...
| Recurring tasks | Cron jobs, CI pipelines | [Scheduled tasks](./overview.md) |
| Computer use | [Enable via `/mcp`](./overview.md) on macOS | [App and screen control](#let-claude-use-your-computer) on macOS and Windows |
...
The sections below cover issues specific to the desktop app. For runtime API errors that appear in the chat such as `API Error: 500`, `529 Overloaded`, `429`, or `Prompt is too long`, see the [Error reference](./overview.md). Those errors and their fixes are the same across the CLI, desktop, and web.
