...
Unlike the [sandboxed Bash tool](./sandboxing.md), computer use runs on your actual desktop with access to whatever you approve. Claude checks each action and flags potential prompt injection from on-screen content, but the trust boundary is different. See the computer use safety guide for best practices.
...
For setup, pairing, and Dispatch settings, see the Dispatch help article. Dispatch requires a Pro or Max plan and is not available on Team or Enterprise plans.
...
Connectors are [MCP servers](./mcp.md) with a graphical setup flow. Use them for quick integration with supported services. For integrations not listed in Connectors, add MCP servers manually via [settings files](./mcp.md#installing-mcp-servers). You can also create custom connectors.
...
Enterprise organizations can require SSO for all users. See [authentication](./authentication.md) for plan-level details and Setting up SSO for SAML and OIDC configuration.
...
| [Third-party providers](./third-party-integrations.md) | Bedrock, Vertex, Foundry | Anthropic’s API by default. Enterprise deployments can configure Vertex AI and gateway providers. See the enterprise configuration guide. |
...
* **Third-party providers**: Desktop connects to Anthropic’s API by default. Enterprise deployments can configure Vertex AI and gateway providers via managed settings. For Bedrock or Foundry, use the [CLI](./quickstart.md).
