...
When executed with `--dangerously-skip-permissions`, dev containers do not prevent a malicious project from exfiltrating anything accessible inside the container, including the Claude Code credentials stored in [`~/.claude`](./overview.md).
...
See [Choose your API provider](./overview.md#choose-your-api-provider) to decide which path fits your organization.
...
By default, the container’s home directory is discarded on rebuild, so engineers must sign in again each time. Claude Code stores its authentication token, user settings, and session history under [`~/.claude`](./overview.md). Mount a named volume at that path to keep this state across rebuilds.
...
Replace `/home/node` with the home directory of your container’s `remoteUser`. If you mount the volume somewhere other than `~/.claude`, set [`CLAUDE_CONFIG_DIR`](./overview.md) to the mount path so Claude Code reads and writes there.
...
To set [environment variables](./overview.md) that apply to every Claude Code session in the container, add them to `containerEnv` in your `devcontainer.json`. The following example opts out of telemetry and error reporting and prevents Claude Code from auto-updating after install:
...
For the full list of policy controls including permission rules, tool restrictions, and MCP server allowlists, see [Set up Claude Code for your organization](./overview.md).
...
* [Set up Claude Code for your organization](./overview.md): choose an authentication provider, decide how policy reaches devices, and plan the rollout
...
* [Explore the `.claude` directory](./overview.md): what the volume mount holds, including credentials, settings, and session history
* [Security model](./security.md): how Claude Code’s permission system, sandboxing, and prompt-injection protections fit together
* [Permission modes](./overview.md): the full range from plan mode to auto mode to bypass, and when to use each
