...
**When:** Session start.**What loads:** Tool names from connected servers. Full JSON schemas stay deferred until Claude needs a specific tool.**Context cost:** [Tool search](./mcp.md#scale-with-mcp-tool-search) is on by default, so idle MCP tools consume minimal context.

Run `/mcp` to see connection status and token costs per server. Claude Code [reconnects to remote servers automatically](./mcp.md#automatic-reconnection) if they drop, and you can disconnect servers you’re not actively using.
