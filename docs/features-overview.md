...
Claude Code combines a model that reasons about your code with [built-in tools](./how-claude-code-works#tools "._how-claude-code-works#tools".md) for file operations, search, execution, and web access. The built-in tools cover most coding tasks. This guide covers the extension layer: features you add to customize what Claude knows, connect it to external services, and automate workflows.

For how the core agentic loop works, see [How Claude Code works](./how-claude-code-works "._how-claude-code-works".md).

**New to Claude Code?** Start with [CLAUDE.md](./memory "._memory".md) for project conventions, then add other extensions [as specific triggers come up](#build-your-setup-over-time "#build-your-setup-over-time").

## [​](#overview "#overview") Overview

Extensions plug into different parts of the agentic loop:

* **[CLAUDE.md](./memory "._memory".md)** adds persistent context Claude sees every session
* **[Skills](./skills "._skills".md)** add reusable knowledge and invocable workflows
* **[Code intelligence](./tools-reference#lsp-tool-behavior "._tools-reference#lsp-tool-behavior".md)** connects Claude to a language server for symbol-level navigation and live type errors
* **[MCP](./mcp "._mcp".md)** connects Claude to external services and tools
* **[Subagents](./sub-agents "._sub-agents".md)** run their own loops in isolated context, returning summaries
* **[Agent teams](./agent-teams "._agent-teams".md)** coordinate multiple independent sessions with shared tasks and peer-to-peer messaging
* **[Hooks](./hooks-guide "._hooks-guide".md)** fire on lifecycle events and can run a script, HTTP request, prompt, or subagent
* **[Plugins](./plugins "._plugins".md)** and **[marketplaces](./plugin-marketplaces "._plugin-marketplaces".md)** package and distribute these features

[Skills](./skills "._skills".md) are the most flexible extension. A skill is a markdown file containing knowledge, workflows, or instructions. You can invoke skills with a command like `/deploy`, or Claude can load them automatically when relevant. Skills can run in your current conversation or in an isolated context via subagents.

## [​](#match-features-to-your-goal "#match-features-to-your-goal") Match features to your goal
...
| **[Agent teams](./agent-teams "._agent-teams".md)** | Coordinate multiple independent Claude Code sessions | Parallel research, new feature development, debugging with competing hypotheses | Spawn reviewers to check security, performance, and tests simultaneously |
| **[Code intelligence](./tools-reference#lsp-tool-behavior "._tools-reference#lsp-tool-behavior".md)** | Language-server navigation and diagnostics | Typed languages, large codebases where grep is slow or imprecise | Jump to a symbol’s definition instead of reading the whole file |
...
| **[Artifact](./artifacts "._artifacts".md)** | Publish session output as a private, interactive web page | Output you want to see or share visually rather than as terminal text | An incident timeline that updates as Claude investigates |

**[Plugins](./plugins "._plugins".md)** are the packaging layer. A plugin bundles skills, hooks, subagents, and MCP servers into a single installable unit. Plugin skills are namespaced (like `/my-plugin:review`) so multiple plugins can coexist. Use plugins when you want to reuse the same setup across multiple repositories or distribute to others via a **[marketplace](./plugin-marketplaces "._plugin-marketplaces".md)**.

### [​](#build-your-setup-over-time "#build-your-setup-over-time") Build your setup over time
...
| Trigger | Add |
| --- | --- |
| Claude gets a convention or command wrong twice | Add it to [CLAUDE.md](./memory "._memory".md) |
| You keep typing the same prompt to start a task | Save it as a user-invocable [skill](./skills "._skills".md) |
| You paste the same playbook or multi-step procedure into chat for the third time | Capture it as a [skill](./skills "._skills".md) |
| You keep copying data from a browser tab Claude can’t see | Connect that system as an [MCP server](./mcp "._mcp".md) |
| Claude reads many files to find where a symbol is defined or used | Install a [code intelligence plugin](./discover-plugins#code-intelligence "._discover-plugins#code-intelligence".md) for your language |
| A side task floods your conversation with output you won’t reference again | Route it through a [subagent](./sub-agents "._sub-agents".md) |
| You want something to happen every time without asking | Write a [hook](./hooks-guide "._hooks-guide".md) |
| A second repository needs the same setup | Package it as a [plugin](./plugins "._plugins".md) |
...
### [​](#compare-similar-features "#compare-similar-features") Compare similar features

Some features can seem similar. For a deeper walkthrough of choosing between them, see [Steering Claude Code: when to use CLAUDE.md, skills, hooks, and subagents](https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more "https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more") on the blog. Here’s how to tell them apart.
...
**Use CLAUDE.md** for instructions every session needs: build commands, test conventions, project architecture.**Use rules** to keep CLAUDE.md focused. Rules with [`paths` frontmatter](./memory#path-specific-rules "._memory#path-specific-rules".md) only load when Claude works with matching files, saving context.**Use skills** for content Claude only needs sometimes, like API documentation or a deployment checklist you trigger with `/<name>`.
...
Agent teams are experimental and disabled by default. See [agent teams](./agent-teams "._agent-teams".md) for setup and current limitations.
...
| **Triggered by** | [Lifecycle events](./hooks#hook-events "._hooks#hook-events".md) such as `PostToolUse` or `SessionStart` | You typing `/<name>`, or Claude matching the description to your task |
...
### [​](#understand-how-features-layer "#understand-how-features-layer") Understand how features layer
...
* **CLAUDE.md files** are additive: all levels contribute content to Claude’s context simultaneously. Files from your working directory and above load at launch; subdirectories load as you work in them. When instructions conflict, Claude uses judgment to reconcile them, with more specific instructions typically taking precedence. See [how CLAUDE.md files load](./memory#how-claude-md-files-load "._memory#how-claude-md-files-load".md).
* **Skills and subagents** override by name: when the same name exists at multiple levels, one definition wins based on priority (managed > user > project for skills; managed > CLI flag > project > user > plugin for subagents). Plugin skills are [namespaced](./plugins#add-skills-to-your-plugin "._plugins#add-skills-to-your-plugin".md) to avoid conflicts. See [skill discovery](./skills#where-skills-live "._skills#where-skills-live".md) and [subagent scope](./sub-agents#choose-the-subagent-scope "._sub-agents#choose-the-subagent-scope".md).
* **MCP servers** override by name: local > project > user. See [MCP scope](./mcp#scope-hierarchy-and-precedence "._mcp#scope-hierarchy-and-precedence".md).
* **Hooks** merge: all registered hooks fire for their matching events regardless of source. See [hooks](./hooks "._hooks".md).

### [​](#combine-features "#combine-features") Combine features
...
## [​](#understand-context-costs "#understand-context-costs") Understand context costs

Every feature you add consumes some of Claude’s context. Too much can fill up your context window, but it can also add noise that makes Claude less effective; skills may not trigger correctly, or Claude may lose track of your conventions. Understanding these trade-offs helps you build an effective setup. For an interactive view of how these features combine in a running session, see [Explore the context window](./context-window "._context-window".md).

### [​](#context-cost-by-feature "#context-cost-by-feature") Context cost by feature
...
\*By default, skill descriptions load at session start so Claude can decide when to use them. Set `disable-model-invocation: true` in a skill’s frontmatter to hide it from Claude entirely until you invoke it manually. This reduces context cost to zero for skills you only trigger yourself. For a skill you didn’t write, set [`skillOverrides`](./skills#override-skill-visibility-from-settings "._skills#override-skill-visibility-from-settings".md) in settings to do the same without editing its file.

### [​](#understand-how-features-load "#understand-how-features-load") Understand how features load
...
The LSP tool is inactive until you install a [code intelligence plugin](./discover-plugins#code-intelligence "._discover-plugins#code-intelligence".md) for your language.
...
* CLAUDE.md and git status, except the built-in Explore and Plan agents [omit both](./sub-agents#what-loads-at-startup "._sub-agents#what-loads-at-startup".md)
...
**When:** On trigger. Hooks fire at specific lifecycle events like tool execution, session boundaries, prompt submission, permission requests, and compaction. See [Hooks](./hooks "._hooks".md) for the full list.**What loads:** Nothing by default. Hooks execute outside the main conversation.**Context cost:** Zero, unless the hook returns output that gets added as messages to your conversation.
