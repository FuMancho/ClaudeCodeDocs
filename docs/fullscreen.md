...
Fullscreen rendering is an opt-in [research preview](#research-preview "#research-preview"). Run `/tui fullscreen` to switch in your current conversation. Behavior may change based on feedback.
...
Run `/tui fullscreen` inside any Claude Code conversation. The CLI saves the [`tui` setting](./settings#available-settings "._settings#available-settings".md) and relaunches into fullscreen with your conversation intact, so you can switch mid-session without losing context. Run `/tui default` to switch back to the classic renderer, or `/tui` with no argument to print which renderer is active.
The relaunched session keeps the conversation as it appears on screen. If you ran [`/rewind`](./checkpointing#rewind-and-summarize "._checkpointing#rewind-and-summarize".md) earlier in the session, the relaunch resumes from the rewound point rather than the longer transcript saved on disk. Before v2.1.207, switching renderers after a rewind restored the conversation the rewind had removed.
...
| Before | Now | Details |
| --- | --- | --- |
| `Cmd+f` or tmux search to find text | `Ctrl+o` for transcript mode, then `/` to search or `[` to write to scrollback | [Search and review the conversation](#search-and-review-the-conversation "#search-and-review-the-conversation") |
| Terminal’s native click-and-drag to select and copy | In-app selection, copies automatically on mouse release | [Use the mouse](#use-the-mouse "#use-the-mouse") |
| `Cmd`-click to open a URL | `Cmd`-click on macOS, `Ctrl`-click elsewhere | [Use the mouse](#use-the-mouse "#use-the-mouse") |

If mouse capture interferes with your workflow, you can [turn it off](#keep-native-text-selection "#keep-native-text-selection") while keeping the flicker-free rendering.

## [​](#use-the-mouse "#use-the-mouse") Use the mouse
...
## [​](#scroll-the-conversation "#scroll-the-conversation") Scroll the conversation
...
* Click the [jump-to-bottom button](#auto-follow "#auto-follow").
...
### [​](#mouse-wheel-scrolling "#mouse-wheel-scrolling") Mouse wheel scrolling
...
## [​](#search-and-review-the-conversation "#search-and-review-the-conversation") Search and review the conversation
...
## [​](#clear-the-conversation "#clear-the-conversation") Clear the conversation
...
## [​](#use-with-tmux "#use-with-tmux") Use with tmux
...
Some terminals block OSC 52 by default. iTerm2 blocks it until you turn on Settings → General → Selection → Applications in terminal may access clipboard; running [`/terminal-setup`](./terminal-config "._terminal-config".md) in iTerm2 enables this for you.
...
### [​](#stale-or-misplaced-text-on-screen "#stale-or-misplaced-text-on-screen") Stale or misplaced text on screen
...
Set [`CLAUDE_CODE_ALT_SCREEN_FULL_REPAINT=1`](./env-vars "._env-vars".md) to repaint every cell on every frame instead of sending incremental updates.
...
On Windows, Claude Code already enables full repaint automatically for background sessions and [agent view](./agent-view "._agent-view".md), so you only need to set the variable for an interactive fullscreen session you launched directly.
...
If you encounter a problem, run `/feedback` inside Claude Code to report it, or open an issue on the [claude-code GitHub repo](https://github.com/anthropics/claude-code/issues "https://github.com/anthropics/claude-code/issues"). Include your terminal emulator name and version.
