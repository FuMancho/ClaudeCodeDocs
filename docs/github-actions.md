...
Claude Code GitHub Actions is built on top of the [Claude Agent SDK](./overview.md), which enables programmatic integration of Claude Code into your applications. You can use the SDK to build custom automation workflows beyond GitHub Actions.

**Claude Opus 4.6 is now available.** Claude Code GitHub Actions default to Sonnet. To use Opus 4.6, configure the [model parameter](#breaking-changes-reference #breaking-changes-reference) to use `claude-opus-4-6`.
...
* This quickstart method is only available for direct Claude API users. If
  ...
  Bedrock & Google Vertex AI](#using-with-aws-bedrock-%26-google-vertex-ai #using-with-aws-bedrock-%26-google-vertex-ai)
  section.
...
#### [​](#for-aws-bedrock #for-aws-bedrock) For AWS Bedrock:
