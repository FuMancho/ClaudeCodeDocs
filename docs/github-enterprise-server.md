...
Return to claude.ai/admin-settings/claude-code and enable [Code Review](./code-review.md#set-up-code-review) and [contribution metrics](./analytics.md#enable-contribution-metrics) for your GHES repositories using the same configuration as github.com.
...
The session runs on Anthropic infrastructure, clones your repository from GHES, and pushes changes back to a branch. Monitor progress with `/tasks` or at claude.ai/code. See [Claude Code on the web](./claude-code-on-the-web.md) for the full remote session workflow including diff review, auto-fix, and routines.
