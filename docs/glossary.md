...
A mode that executes a single prompt and exits without a conversational session, invoked with `-p` or `--print`. Used for CI, scripts, and piping. The [Agent SDK](./overview.md) is the Python and TypeScript equivalent. Formerly called headless mode.
