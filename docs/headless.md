...
The [Agent SDK](./overview.md) gives you the same tools, agent loop, and context management that power Claude Code. It’s available as a CLI for scripts and CI/CD, or as [Python](./overview.md) and [TypeScript](./overview.md) packages for full programmatic control.
...
When [`CLAUDE_CODE_SYNC_PLUGIN_INSTALL`](./overview.md) is set, Claude Code emits `system/plugin_install` events while marketplace plugins install before the first turn. Use these to surface install progress in your own UI.
...
For programmatic streaming with callbacks and message objects, see [Stream responses in real-time](./overview.md) in the Agent SDK documentation.
...
User-invoked [skills](./skills.md) like `/commit` and [built-in commands](./overview.md) are only available in interactive mode. In `-p` mode, describe the task you want to accomplish instead.
...
* [Agent SDK quickstart](./overview.md): build your first agent with Python or TypeScript
