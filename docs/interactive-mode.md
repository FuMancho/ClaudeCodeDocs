...
Keyboard shortcuts may vary by platform and terminal. In [fullscreen rendering](./fullscreen "._fullscreen".md), press `?` in the transcript viewer to see available shortcuts there.**macOS users**: Option/Alt key shortcuts (`Alt+B`, `Alt+F`, `Alt+Y`, `Alt+P`) require configuring Option as Meta in your terminal:
...
| `Ctrl+X Ctrl+K` | Stop all running [background subagents](./sub-agents#run-subagents-in-foreground-or-background "._sub-agents#run-subagents-in-foreground-or-background".md) in this session. Press twice within 3 seconds to confirm | Subagent control |
...
| `Ctrl+T` | Toggle Claude’s task checklist | Show or hide [Claude’s to-do checklist](#task-list "#task-list") in the status area. This is not the background-task view; use [`/tasks`](./commands "._commands".md) to see running shells and subagents |
...
| `Alt+Y` (after `Ctrl+Y`) | Cycle paste history | After pasting, cycle through previously deleted text. Requires [Option as Meta](#keyboard-shortcuts "#keyboard-shortcuts") on macOS |
| `Alt+B` | Move cursor back one word | Word navigation. Requires [Option as Meta](#keyboard-shortcuts "#keyboard-shortcuts") on macOS |
| `Alt+F` | Move cursor forward one word | Word navigation. Requires [Option as Meta](#keyboard-shortcuts "#keyboard-shortcuts") on macOS |
| `Ctrl+_` or `Ctrl+Shift+-` | Undo last input edit | Restores the previous input text and cursor position |

### [​](#theme-and-display "#theme-and-display") Theme and display
...
### [​](#multiline-input "#multiline-input") Multiline input
...
### [​](#quick-commands "#quick-commands") Quick commands
...
When the transcript viewer is open (toggled with `Ctrl+O`), these shortcuts are available. Run `/tui` with no argument to check which renderer is active. In [fullscreen rendering](./fullscreen "._fullscreen".md), press `?` to show the full shortcut reference panel inside the viewer. `Ctrl+E` can be rebound via [`transcript:toggleShowAll`](./keybindings "._keybindings".md).
...
| `Ctrl+E` | Toggle show all content. Available in the default renderer only, not in [fullscreen rendering](./fullscreen "._fullscreen".md) |
...
In [fullscreen rendering](./fullscreen#use-the-mouse "._fullscreen#use-the-mouse".md), the `/` command and `@` file suggestion lists also respond to the mouse: hovering highlights a row and clicking accepts it.
...
### [​](#remap-insert-mode-key-sequences "#remap-insert-mode-key-sequences") Remap INSERT-mode key sequences

The [`vimInsertModeRemaps`](./settings#available-settings "._settings#available-settings".md) setting maps a two-key INSERT-mode sequence to Escape, so a mapping like `jj` returns you to NORMAL mode. Requires Claude Code v2.1.208 or later.
...
Claude Code reads this setting from your user settings file, the `--settings` flag, and [managed settings](./permissions#managed-settings "._permissions#managed-settings".md) only. Entries in a project’s `.claude/settings.json` or `.claude/settings.local.json` are ignored, so a checked-out repository can’t remap your keystrokes.

### [​](#navigation-normal-mode "#navigation-normal-mode") Navigation (NORMAL mode)
...
### [​](#editing-normal-mode "#editing-normal-mode") Editing (NORMAL mode)
...
### [​](#text-objects-normal-mode "#text-objects-normal-mode") Text objects (NORMAL mode)
...
### [​](#reverse-search-with-ctrl+r "#reverse-search-with-ctrl+r") Reverse search with Ctrl+R

Press `Ctrl+R` to interactively search through your command history. In [fullscreen rendering](./fullscreen "._fullscreen".md), `Ctrl+R` opens a search dialog instead: type to filter, press `Up` and `Down` to move through matches, and press `Ctrl+S` to cycle the scope through this session, this project, and all projects. Press `Enter` or `Tab` to place a match in the prompt input, or `Esc` to cancel. The steps below describe the default inline search:
...
## [​](#background-bash-commands "#background-bash-commands") Background Bash commands

Claude Code supports running Bash commands in the background, allowing you to continue working while long-running processes execute.

### [​](#how-backgrounding-works "#how-backgrounding-works") How backgrounding works
...
* Background tasks are automatically cleaned up when Claude Code exits. Backgrounding the session instead of exiting it hands them to the background session, where they keep running. See [background a running session](./agent-view#from-inside-a-session "._agent-view#from-inside-a-session".md)
* Background tasks are automatically terminated if output exceeds 5GB, with a note in stderr explaining why
* As of v2.1.193, on macOS and Linux, running background tasks are terminated when the operating system signals memory pressure, provided the session has been idle for at least 30 minutes with no turn or subagent running. Set [`CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP`](./env-vars "._env-vars".md) to `1` to turn this off
...
### [​](#shell-mode-with-prefix "#shell-mode-with-prefix") Shell mode with `!` prefix
...
As of v2.1.186, Claude responds to the command output automatically once it lands in the transcript, so you can run `! npm test` and get an explanation of the failures without a second prompt. The response costs the same as sending a normal prompt. To restore the earlier behavior where the output is added to context without a response, set [`respondToBashCommands`](./settings#available-settings "._settings#available-settings".md) to `false` in `settings.json`. Before v2.1.186, shell mode always added output to context without a response.
...
In print mode they are off by default. Pass [`--prompt-suggestions`](./cli-reference#cli-flags "._cli-reference#cli-flags".md) with `-p "<prompt>" --output-format stream-json --verbose` to emit a `prompt_suggestion` message after each turn instead.
...
## [​](#side-questions-with-/btw "#side-questions-with-/btw") Side questions with /btw
...
| `f` | Fork into a new session. The fork inherits the parent conversation plus this question and answer as real transcript turns, so you can continue with full tool access. The original session is preserved under [`/resume`](./commands "._commands".md). Available in local sessions only |
...
The task list is Claude’s to-do checklist: items Claude created to plan multi-step work, with indicators showing what’s pending, in progress, or complete. It’s separate from the background-task view. To see running shells and subagents, use [`/tasks`](./commands "._commands".md) instead.
...
## [​](#session-recap "#session-recap") Session recap
...
## [​](#pr-review-status "#pr-review-status") PR review status
