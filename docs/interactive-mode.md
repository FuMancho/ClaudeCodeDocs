...
Keyboard shortcuts may vary by platform and terminal. In [fullscreen rendering](./overview.md), press `?` in the transcript viewer to see available shortcuts there.**macOS users**: Option/Alt key shortcuts (`Alt+B`, `Alt+F`, `Alt+Y`, `Alt+M`, `Alt+P`) require configuring Option as Meta in your terminal:
...
| `Shift+Tab` or `Alt+M` (some configurations) | Cycle permission modes | Cycle through `default`, `acceptEdits`, `plan`, and any modes you have enabled, such as `auto` or `bypassPermissions`. See [permission modes](./overview.md). |
...
When the transcript viewer is open (toggled with `Ctrl+O`), these shortcuts are available. In [fullscreen rendering](./overview.md), press `?` to show the full shortcut reference panel inside the viewer. `Ctrl+E` can be rebound via [`transcript:toggleShowAll`](./keybindings.md).

| Shortcut | Description |
| --- | --- |
| `?` | Toggle the keyboard shortcut help panel. Requires [fullscreen rendering](./overview.md) |
| `{` / `}` | Jump to the previous or next user prompt, like vim paragraph motion. Requires [fullscreen rendering](./overview.md) |
| `Ctrl+E` | Toggle show all content |
| `[` | Write the full conversation to your terminal’s native scrollback so `Cmd+F`, tmux copy mode, and other native tools can search it. Requires [fullscreen rendering](./overview.md#search-and-review-the-conversation) |
| `v` | Write the conversation to a temporary file and open it in `$VISUAL` or `$EDITOR`. Requires [fullscreen rendering](./overview.md) |
...
| Shortcut | Description | Notes |
| --- | --- | --- |
| Hold or tap `Space` | Voice dictation | Requires [voice dictation](./overview.md) to be enabled. Hold to record, or run `/voice tap` for tap-to-toggle. [Rebindable](./overview.md#rebind-the-dictation-key) |
...
See the [commands reference](./overview.md) for the full list of commands included in Claude Code.
...
To disable all background task functionality, set the `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` environment variable to `1`. See [Environment variables](./overview.md) for details.
