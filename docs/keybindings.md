...
Actions follow a `namespace:action` format, such as `chat:submit` to send a message or `app:toggleTasks` to show the task list. Each context has specific actions available.
