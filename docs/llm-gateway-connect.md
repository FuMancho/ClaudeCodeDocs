...
The [Agent SDK](./agent-sdk_overview.md) has no gateway-specific options; it passes environment variables to the Claude Code process it spawns. Each SDK accepts an `env` option that sets the spawned process’s environment, and the TypeScript and Python SDKs treat it differently:
