...
The [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction "https://modelcontextprotocol.io/introduction") lets Claude Code use tools beyond its built-in set, such as searching an issue tracker, querying a database, or controlling a web browser. These tools come from MCP servers, which run on your machine or as hosted services.
...
You can also add MCP servers from other surfaces, including the desktop app, VS Code, and the web. See [Connect from other surfaces](#connect-from-other-surfaces "#connect-from-other-surfaces").

For every way to connect and configure MCP servers in Claude Code, see the [MCP reference](./mcp "._mcp".md).

## [​](#before-you-begin "#before-you-begin") Before you begin

Make sure you have:

* [Claude Code installed](./quickstart "._quickstart".md) and authenticated
* A terminal open in a project directory. Any directory works, including an empty one.

## [​](#add-and-verify-a-server "#add-and-verify-a-server") Add and verify a server
...
The steps are the same for any server: add it, check the connection status, then use it in a session, with an optional cleanup step at the end. Some servers add a step, like a browser sign-in, shown in [Additional MCP server examples](#additional-mcp-server-examples "#additional-mcp-server-examples"). For more servers to connect, browse the [Anthropic Directory](./mcp#find-and-build-mcp-servers "._mcp#find-and-build-mcp-servers".md).
...
| `! Needs authentication` | The server is reachable but needs a browser sign-in, or a token passed with `--header`. See [Connect a server that requires sign-in](#connect-a-server-that-requires-sign-in "#connect-a-server-that-requires-sign-in") |
| `✘ Failed to connect` | Server didn’t respond. See [Troubleshooting](#troubleshooting "#troubleshooting") |
| `✘ Connection error` | The connection attempt threw an error. See [Troubleshooting](#troubleshooting "#troubleshooting") |
| `` ⏸ Pending approval (run `claude` to approve) `` | A project-scoped server you haven’t approved yet. See [Edit .mcp.json directly](#edit-mcp-json-directly "#edit-mcp-json-directly") |
...
Each connected server takes some space in [Claude’s context window](./how-claude-code-works#the-context-window "._how-claude-code-works#the-context-window".md) because its tool names and server instructions load into every session. Removing servers you no longer use keeps that space free.

## [​](#where-servers-are-saved "#where-servers-are-saved") Where servers are saved
...
* [Add a local server](#add-a-local-server "#add-a-local-server"): run a program on your machine instead of connecting to a URL.
* [Edit `.mcp.json` directly](#edit-mcp-json-directly "#edit-mcp-json-directly"): write the JSON entry yourself instead of using the command.
* [Connect a server that requires sign-in](#connect-a-server-that-requires-sign-in "#connect-a-server-that-requires-sign-in"): add a hosted server that needs a browser sign-in before its tools work.

### [​](#find-your-configuration-on-disk "#find-your-configuration-on-disk") Find your configuration on disk
...
On Windows, `~/.claude.json` resolves to `%USERPROFILE%\.claude.json`, typically `C:\Users\YourName\.claude.json`. If you’ve set [`CLAUDE_CONFIG_DIR`](./env-vars "._env-vars".md), Claude Code reads `.claude.json` from inside that directory instead.
Run `claude mcp get claude-code-docs` to see which scope holds a server’s definition. For how the scopes interact when the same server is defined in more than one, see [MCP installation scopes](./mcp#mcp-installation-scopes "._mcp#mcp-installation-scopes".md).

## [​](#change-server-scope "#change-server-scope") Change server scope
...
### [​](#use-a-server-in-all-your-projects "#use-a-server-in-all-your-projects") Use a server in all your projects
...
### [​](#share-a-server-with-your-team "#share-a-server-with-your-team") Share a server with your team
...
## [​](#additional-mcp-server-examples "#additional-mcp-server-examples") Additional MCP server examples
...
### [​](#add-a-local-server "#add-a-local-server") Add a local server
...
The [Playwright MCP server](https://github.com/microsoft/playwright-mcp "https://github.com/microsoft/playwright-mcp") is a good one to try: it gives Claude a browser it can navigate, click, and read, and it needs no account. It runs through `npx`, so it requires [Node.js](https://nodejs.org/en/download "https://nodejs.org/en/download") 18 or later.
...
The first check can show `✘ Failed to connect` while `npx` downloads the package, so wait a moment and run it again. Once the download finishes, the status changes to `✔ Connected`. If it still shows `✘ Failed to connect` after a couple of retries, see [Troubleshooting](#troubleshooting "#troubleshooting").
...
### [​](#connect-a-server-that-requires-sign-in "#connect-a-server-that-requires-sign-in") Connect a server that requires sign-in
...
The steps below use Sentry as the example. To connect a different service, substitute its URL, which you can find in the [Anthropic Directory](./mcp#find-and-build-mcp-servers "._mcp#find-and-build-mcp-servers".md) or the service’s documentation.
...
Select `sentry` from the list, press Enter, and choose `Authenticate`. Your browser opens to Sentry’s sign-in page. Approve the connection there.Back in Claude Code, the server’s status changes to connected. If sign-in fails or the browser doesn’t open, see [Troubleshooting](#troubleshooting "#troubleshooting").
...
Servers that authenticate with a static token instead of OAuth take the token at add time with `--header "Authorization: Bearer <token>"`. See the [GitHub example](./mcp#example-connect-to-github-for-code-reviews "._mcp#example-connect-to-github-for-code-reviews".md) for a worked version.

## [​](#edit-mcp-json-directly "#edit-mcp-json-directly") Edit .mcp.json directly

Every file in the [scope table](#find-your-configuration-on-disk "#find-your-configuration-on-disk") uses the same JSON format for server entries. This section edits `.mcp.json`, the project-scope file. It’s the one most worth writing by hand because it’s checked into the repository, where it doubles as configuration-as-code for your team.
...
Once you’ve approved, run `/mcp` and check that the servers show as connected. If one shows an error instead, see [Troubleshooting](#troubleshooting "#troubleshooting").

## [​](#connect-from-other-surfaces "#connect-from-other-surfaces") Connect from other surfaces

This guide uses the `claude mcp` CLI commands, but every Claude Code surface can connect to MCP servers:

* **Claude Code desktop app**: add servers through the [Connectors UI](./desktop#connect-external-tools "._desktop#connect-external-tools".md).
...
* **VS Code**: see [Connect to external tools with MCP](./vs-code#connect-to-external-tools-with-mcp "._vs-code#connect-to-external-tools-with-mcp".md).
* **Claude Code on the web**: reads `.mcp.json` from your repository. See [Edit .mcp.json directly](#edit-mcp-json-directly "#edit-mcp-json-directly").
* **Claude.ai**: connectors you add at [claude.ai/customize/connectors](https://claude.ai/customize/connectors "https://claude.ai/customize/connectors") load automatically in the CLI when you sign in with that account. See [Use MCP servers from Claude.ai](./mcp#use-mcp-servers-from-claude-ai "._mcp#use-mcp-servers-from-claude-ai".md).
...
The server took longer than the default 30-second startup timeout. A stdio server’s first run can be slow while `npx` downloads the package. Increase the limit with the [`MCP_TIMEOUT`](./env-vars "._env-vars".md) environment variable, in milliseconds:
...
Run `/mcp`, select the server, and choose `Authenticate` again. If the browser doesn’t open automatically, copy the URL shown in the terminal and open it manually. See [Authenticate with remote MCP servers](./mcp#authenticate-with-remote-mcp-servers "._mcp#authenticate-with-remote-mcp-servers".md) for fixed callback ports and pre-configured credentials.

## [​](#next-steps "#next-steps") Next steps

With one server connected, explore the rest of what MCP enables:

* [Find more MCP servers](./mcp#find-and-build-mcp-servers "._mcp#find-and-build-mcp-servers".md) in the Anthropic Directory
* [Share servers with your team](./mcp#mcp-installation-scopes "._mcp#mcp-installation-scopes".md) using installation scopes
* [Manage MCP access for an organization](./managed-mcp "._managed-mcp".md) with managed settings and policy controls
* [Reference MCP resources](./mcp#use-mcp-resources "._mcp#use-mcp-resources".md) in prompts with @ mentions
* [Run MCP prompts as commands](./mcp#use-mcp-prompts-as-commands "._mcp#use-mcp-prompts-as-commands".md) from the `/` menu
* [Build your own server](https://modelcontextprotocol.io/quickstart/server "https://modelcontextprotocol.io/quickstart/server") with the MCP SDK