...
When a model switch is requested through the [Agent SDK](./agent-sdk_overview.md) `setModel()` method or by an app such as the [Desktop app](./desktop.md) that runs the Claude Code CLI for you, Claude Code checks that the string is one it recognizes before saving it. This check requires Claude Code v2.1.200 or later. On the Anthropic API, Claude Code recognizes:
...
* **Subagent models**: the `model` field in [subagent](./sub-agents.md#choose-a-model) frontmatter, the Agent tool’s `model` parameter, `CLAUDE_CODE_SUBAGENT_MODEL`, and, on v2.1.197 and earlier, the model picker in the `/agents` wizard
