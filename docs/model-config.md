...
On the Anthropic API and [Claude Platform on AWS](./overview.md), `opus` resolves to Opus 4.7 and `sonnet` resolves to Sonnet 4.6. On Bedrock, Vertex, and Foundry, `opus` resolves to Opus 4.6 and `sonnet` resolves to Sonnet 4.5; newer models are available on those providers by selecting the full model name explicitly or setting `ANTHROPIC_DEFAULT_OPUS_MODEL` or `ANTHROPIC_DEFAULT_SONNET_MODEL`.
...
On Opus 4.6 and Sonnet 4.6, you can set `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1` to revert to the previous fixed thinking budget controlled by `MAX_THINKING_TOKENS`. See [environment variables](./overview.md).
...
| Disable regardless of effort | Set [`MAX_THINKING_TOKENS=0`](./overview.md). Other values apply only with a [fixed thinking budget](#adaptive-reasoning-and-fixed-thinking-budgets) |
...
To disable 1M context entirely, set `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`. This removes 1M model variants from the model picker. See [environment variables](./overview.md).
...
When deploying Claude Code through [Bedrock](./amazon-bedrock.md), [Vertex AI](./google-vertex-ai.md), [Foundry](./microsoft-foundry.md), or [Claude Platform on AWS](./overview.md), pin model versions before rolling out to users.
