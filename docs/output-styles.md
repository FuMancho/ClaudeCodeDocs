...
For instructions about your project, conventions, or codebase, use [CLAUDE.md](./memory.md "/docs/en/memory") instead.
...
Run `/config` and select **Output style** to pick a style from a menu. Your selection is saved to `.claude/settings.local.json` at the [local project level](./settings.md "/docs/en/settings").
...
Output style is part of the system prompt, which Claude Code reads once at session start. Changes take effect after `/clear` or a new session. See [How Claude Code uses prompt caching](./prompt-caching.md#changing-output-style "/docs/en/prompt-caching#changing-output-style") for what an output style change does to the cache.
...
* Managed policy: `.claude/output-styles` inside the [managed settings directory](./settings.md#settings-files "/docs/en/settings#settings-files")
...
[Plugins](./plugins-reference.md "/docs/en/plugins-reference") can also ship output styles in an `output-styles/` directory.
...
| [CLAUDE.md](./memory.md "/docs/en/memory") | Adds a user message after the system prompt | Claude should always know your project conventions and codebase context |
...
| [Agents](./sub-agents.md "/docs/en/sub-agents") | Runs a subagent with its own system prompt, model, and tools | You want a separately scoped helper for a focused task |
| [Skills](./skills.md "/docs/en/skills") | Loads task-specific instructions when invoked or relevant | You have a reusable workflow |

## [​](#related-resources "#related-resources") Related resources

* [Settings](./settings.md "/docs/en/settings"): where the `outputStyle` field lives and how settings precedence works
* [Permission modes](./permission-modes.md "/docs/en/permission-modes"): how the Proactive style compares to auto mode
* [Plugins](./plugins.md "/docs/en/plugins"): package and distribute output styles alongside skills, hooks, and agents
* [Debug your configuration](./debug-your-config.md "/docs/en/debug-your-config"): diagnose why an output style isn’t taking effect