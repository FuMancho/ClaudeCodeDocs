...
1. **Create plugins**: build one or more plugins with skills, agents, hooks, MCP servers, or LSP servers. This guide assumes you already have plugins to distribute; see [Create plugins](./plugins "._plugins".md) for details on how to create them.
2. **Create the marketplace file**: define a `marketplace.json` that lists your plugins and where to find them. See [Create the marketplace file](#create-the-marketplace-file "#create-the-marketplace-file").
...
## [​](#walkthrough-create-a-local-marketplace "#walkthrough-create-a-local-marketplace") Walkthrough: create a local marketplace
...
Each plugin entry needs at minimum a `name` and a `source` that tells Claude Code where to fetch it from. See the [full schema](#marketplace-schema "#marketplace-schema") below for all available fields.
...
## [​](#marketplace-schema "#marketplace-schema") Marketplace schema
...
### [​](#optional-fields "#optional-fields") Optional fields
...
Each plugin entry in the `plugins` array describes a plugin and where to find it. You can include any field from the [plugin manifest schema](./plugins-reference#plugin-manifest-schema "._plugins-reference#plugin-manifest-schema".md), such as `description`, `version`, `author`, `commands`, and `hooks`, plus these marketplace-specific fields: `source`, `category`, `tags`, `strict`, and `relevance`.
...
| `source` | string|object | Where to fetch the plugin from (see [Plugin sources](#plugin-sources "#plugin-sources") below) |

### [​](#optional-plugin-fields "#optional-plugin-fields") Optional plugin fields
...
| `version` | string | Plugin version. If set (here or in `plugin.json`), the plugin is pinned to this string and users only receive updates when it changes. Omit to fall back to the git commit SHA. See [Version resolution](#version-resolution-and-release-channels "#version-resolution-and-release-channels"). |
...
| `relevance` | object | Signals that tell Claude Code when to suggest this plugin to users. Takes effect only for marketplaces an administrator allowlists in managed settings. See [Recommend plugins for your org](./plugin-relevance "._plugin-relevance".md). Requires Claude Code v2.1.152 or later. |
| `defaultEnabled` | boolean | Whether the plugin is enabled after install (default: true). Set to `false` to install the plugin disabled until the user opts in. Takes precedence over the same field in the plugin’s `plugin.json`. See [Default enablement](./plugins-reference#default-enablement "._plugins-reference#default-enablement".md). Requires Claude Code v2.1.154 or later. |
...
## [​](#plugin-sources "#plugin-sources") Plugin sources
...
### [​](#relative-paths "#relative-paths") Relative paths
...
### [​](#git-repositories "#git-repositories") Git repositories
...
### [​](#git-subdirectories "#git-subdirectories") Git subdirectories
...
### [​](#advanced-plugin-entries "#advanced-plugin-entries") Advanced plugin entries
...
* **`${CLAUDE_PLUGIN_ROOT}`**: use this variable in hook commands and MCP server configs to reference files within the plugin’s installation directory. This is necessary because plugins are copied to a cache location when installed.
  + See the [substitution table](./plugins-reference#environment-variables "._plugins-reference#environment-variables".md) for which config fields substitute it per server type
  + For dependencies or state that should survive plugin updates, use [`${CLAUDE_PLUGIN_DATA}`](./plugins-reference#persistent-data-directory "._plugins-reference#persistent-data-directory".md) instead
...
## [​](#host-and-distribute-marketplaces "#host-and-distribute-marketplaces") Host and distribute marketplaces

### [​](#host-on-github-recommended "#host-on-github-recommended") Host on GitHub (recommended)
...
### [​](#host-on-other-git-services "#host-on-other-git-services") Host on other git services
...
### [​](#test-locally-before-distribution "#test-locally-before-distribution") Test locally before distribution
...
### [​](#pre-populate-plugins-for-containers "#pre-populate-plugins-for-containers") Pre-populate plugins for containers
...
For organizations requiring strict control over plugin sources, administrators can restrict which plugin marketplaces users are allowed to add using the [`strictKnownMarketplaces`](./settings#strictknownmarketplaces "._settings#strictknownmarketplaces".md) setting in managed settings. To also reject the CLI flags that sideload plugins, agents, and MCP servers for a single run, pair it with [`disableSideloadFlags`](./settings#available-settings "._settings#available-settings".md). To allowlist which marketplaces’ plugins can appear as contextual install suggestions, set [`pluginSuggestionMarketplaces`](./settings#available-settings "._settings#available-settings".md).
...
#### [​](#common-configurations "#common-configurations") Common configurations
...
`strictKnownMarketplaces` restricts what users can add, but doesn’t register marketplaces on its own. To make allowed marketplaces available automatically without users running `/plugin marketplace add`, pair it with [`extraKnownMarketplaces`](./settings#extraknownmarketplaces "._settings#extraknownmarketplaces".md) in the same `managed-settings.json`. See [Using both together](./settings#strictknownmarketplaces "._settings#strictknownmarketplaces".md).
...
Because `strictKnownMarketplaces` is set in [managed settings](./settings#settings-files "._settings#settings-files".md), individual users and project configurations can’t override these restrictions.
...
### [​](#rename-or-remove-a-plugin "#rename-or-remove-a-plugin") Rename or remove a plugin

A plugin’s `name` is its stable identifier. Users reference it in `enabledPlugins`, `pluginConfigs`, and `/plugin install` commands, so changing it breaks every existing install. To change the label shown in the UI without breaking installs, set [`displayName`](#optional-plugin-fields "#optional-plugin-fields") and keep `name` unchanged.
...
### [​](#plugin-marketplace-add "#plugin-marketplace-add") Plugin marketplace add
...
### [​](#plugin-marketplace-list "#plugin-marketplace-list") Plugin marketplace list
...
### [​](#marketplace-validation-errors "#marketplace-validation-errors") Marketplace validation errors
...
| `plugins[0].source: Path contains ".."` | Source path contains `..` | Use paths relative to the marketplace root without `..`. See [Relative paths](#relative-paths "#relative-paths") |
...
### [​](#plugin-installation-failures "#plugin-installation-failures") Plugin installation failures
...
### [​](#private-repository-authentication-fails "#private-repository-authentication-fails") Private repository authentication fails
...
* If the re-clone times out on a large repository, increase the limit with [`CLAUDE_CODE_PLUGIN_GIT_TIMEOUT_MS`](#git-operations-time-out "#git-operations-time-out")
* Configure a [git URL rewrite](#private-repositories "#private-repositories") scoped to the marketplace repository so the background pull authenticates directly
* Or update private marketplaces manually with `/plugin marketplace update <name>`, which uses your credentials

### [​](#marketplace-updates-fail-in-offline-environments "#marketplace-updates-fail-in-offline-environments") Marketplace updates fail in offline environments

**Symptoms**: Marketplace `git pull` fails in the background and Claude Code repeatedly attempts a re-clone that can’t succeed.
**Cause**: By default, when a `git pull` fails, Claude Code attempts a re-clone from scratch. In offline or airgapped environments, re-cloning fails the same way, and the restore of the previous cache afterward is best-effort. The refresh runs in the background after startup, so it doesn’t delay startup, but each session repeats the failed attempts and each git operation can wait out the [120-second timeout](#git-operations-time-out "#git-operations-time-out").
...
### [​](#plugins-with-relative-paths-fail-in-url-based-marketplaces "#plugins-with-relative-paths-fail-in-url-based-marketplaces") Plugins with relative paths fail in URL-based marketplaces
...
### [​](#files-not-found-after-installation "#files-not-found-after-installation") Files not found after installation
