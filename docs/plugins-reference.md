...
| `rust-lsp` | rust-analyzer | [See rust-analyzer installation](https://rust-analyzer.github.io/manual.html#installation https://rust-analyzer.github.io/manual.html#installation) |
...
For `skills`, `commands`, `agents`, and `outputStyles`, custom paths replace the default directory. If the manifest specifies `skills`, the default `skills/` directory is not scanned. [Hooks](#hooks #hooks), [MCP servers](#mcp-servers #mcp-servers), and [LSP servers](#lsp-servers #lsp-servers) have different semantics for handling multiple sources.
...
### [​](#working-with-external-dependencies #working-with-external-dependencies) Working with external dependencies
