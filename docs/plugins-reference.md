...
* `prompt`: evaluate a prompt with an LLM (uses `$ARGUMENTS` markers for context)
...
Plugin monitors use the same mechanism as the [Monitor tool](./overview.md#monitor-tool) and share its availability constraints. They run only in interactive CLI sessions, run unsandboxed at the same trust level as [hooks](#hooks), and are skipped on hosts where the Monitor tool is unavailable.
...
| `experimental.monitors` | string|array | Background [Monitor](./overview.md#monitor-tool) configurations that start automatically when the plugin is active. See [Monitors](#monitors) | `"./monitors.json"` |
...
| `dependencies` | array | Other plugins this plugin requires, optionally with semver version constraints. See [Constrain plugin dependency versions](./overview.md) | `[{ "name": "secrets-vault", "version": "~2.1.0" }]` |
...
Remove auto-installed plugin dependencies that are no longer required by any installed plugin. Dependencies that Claude Code pulled in to satisfy another plugin’s [`dependencies`](./overview.md) field are removed; plugins you installed directly are never touched.
...
Create a release git tag for the plugin in the current directory. Run from inside the plugin’s folder. See [Tag plugin releases](./overview.md#tag-plugin-releases-for-version-resolution).
