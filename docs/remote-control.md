...
* **Ultraplan disconnects Remote Control**: starting an [ultraplan](./overview.md) session disconnects any active Remote Control session because both features occupy the claude.ai/code interface and only one can be connected at a time.
...
| [Channels](./overview.md) | Push events from a chat app like Telegram or Discord, or your own server | Your machine (CLI) | [Install a channel plugin](./overview.md#quickstart) or [build your own](./overview.md) | Reacting to external events like CI failures or chat messages |
...
| [Scheduled tasks](./overview.md) | Set a schedule | [CLI](./overview.md), [Desktop](./overview.md), or [cloud](./overview.md) | Pick a frequency | Recurring automation like daily reviews |
...
* [Ultraplan](./overview.md): launch a cloud planning session from your terminal and review the plan in your browser
* [Channels](./overview.md): forward Telegram, Discord, or iMessage into a session so Claude reacts to messages while you’re away
