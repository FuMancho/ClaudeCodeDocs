...
The plugin is the in-session companion to [Code Review](./code-review "._code-review".md), which runs on pull requests. This plugin reduces what reaches the PR. Code Review catches what does. For how the plugin layers with on-demand review and CI scanning, see [How this fits with other security tools](#how-this-fits-with-other-security-tools "#how-this-fits-with-other-security-tools").
...
## [​](#install-the-plugin "#install-the-plugin") Install the plugin

In a terminal Claude Code session, install from the [official Anthropic marketplace](./discover-plugins#official-anthropic-marketplace "._discover-plugins#official-anthropic-marketplace".md):
...
* **Claude desktop app, local or SSH session**: open the [plugin browser](./desktop#install-plugins "._desktop#install-plugins".md) by clicking the **+** button next to the prompt, then **Plugins**, then **Add plugin**
* **Claude Code on the web or a desktop cloud session**: declare the plugin in `.claude/settings.json` as shown under [Enable in cloud sessions](#enable-in-cloud-sessions-and-shared-repositories "#enable-in-cloud-sessions-and-shared-repositories")
...
### [​](#enable-in-cloud-sessions-and-shared-repositories "#enable-in-cloud-sessions-and-shared-repositories") Enable in cloud sessions and shared repositories

User-scoped plugins do not carry into [Claude Code on the web](./claude-code-on-the-web "._claude-code-on-the-web".md), because those sessions run on Anthropic infrastructure rather than your machine. To enable the plugin there, or to turn it on for everyone who clones a repository, declare it in the project’s checked-in settings:
...
Administrators can enable the plugin organization-wide by setting [`enabledPlugins`](./settings#plugin-settings "._settings#plugin-settings".md) in [managed settings](./admin-setup "._admin-setup".md).

## [​](#what-the-plugin-checks "#what-the-plugin-checks") What the plugin checks

The plugin reviews Claude’s work at three points, each at a different depth:

* [On each file edit](#on-each-file-edit "#on-each-file-edit"): a fast pattern match for risky calls, with no model call
* [At the end of each turn](#at-the-end-of-each-turn "#at-the-end-of-each-turn"): a background model review of everything that turn changed
* [On each commit or push Claude makes](#on-each-commit-or-push-claude-makes "#on-each-commit-or-push-claude-makes"): a deeper agentic review that reads surrounding code

You can extend each layer by [adding your own rules](#add-your-own-rules "#add-your-own-rules"). Built-in checks cannot be removed individually, but you can [disable each layer](#disable-or-uninstall "#disable-or-uninstall") independently.

### [​](#on-each-file-edit "#on-each-file-edit") On each file edit
...
You can [add your own patterns](#add-custom-per-edit-patterns "#add-custom-per-edit-patterns") to this layer with a `security-patterns.yaml` file.

### [​](#at-the-end-of-each-turn "#at-the-end-of-each-turn") At the end of each turn
...
### [​](#on-each-commit-or-push-claude-makes "#on-each-commit-or-push-claude-makes") On each commit or push Claude makes
...
### [​](#review-independence-and-limits "#review-independence-and-limits") Review independence and limits
...
None of the layers block writes or commits. Findings reach the writing Claude as instructions, Claude addresses them in the conversation, and the review model can miss issues. Treat the plugin as one layer of defense in depth, not a complete security solution. See [How this fits with other security tools](#how-this-fits-with-other-security-tools "#how-this-fits-with-other-security-tools").

## [​](#add-your-own-rules "#add-your-own-rules") Add your own rules
...
### [​](#add-guidance-for-the-model-backed-reviews "#add-guidance-for-the-model-backed-reviews") Add guidance for the model-backed reviews
...
These rules are guidance for the reviewer, not deterministic guardrails. The plugin surfaces violations as findings for Claude to fix, but it does not block writes or guarantee every violation is caught. The guidance is additive only: a rule that says to ignore a vulnerability class does not suppress those findings. For hard enforcement, pair the plugin with a [hook that blocks the edit](./hooks-guide#block-edits-to-protected-files "._hooks-guide#block-edits-to-protected-files".md) or a CI check.

### [​](#add-custom-per-edit-patterns "#add-custom-per-edit-patterns") Add custom per-edit patterns

Create `.claude/security-patterns.yaml` to add regex or substring rules to the [per-edit pattern check](#on-each-file-edit "#on-each-file-edit"). These run as deterministic string matches alongside the built-in patterns:
...
### [​](#rule-file-lookup-locations "#rule-file-lookup-locations") Rule file lookup locations
...
The [per-edit pattern check](#on-each-file-edit "#on-each-file-edit") makes no model call and adds no cost. The [end-of-turn](#at-the-end-of-each-turn "#at-the-end-of-each-turn") and [commit](#on-each-commit-or-push-claude-makes "#on-each-commit-or-push-claude-makes") reviews each spend additional model usage that counts toward your [usage](./costs "._costs".md) like any other Claude request. The commit review is agentic and may take several model turns per commit, capped at 20 reviews per rolling hour. Expect roughly one review call per turn that changes files and one deeper review per commit, both subject to the caps above.
...
## [​](#disable-or-uninstall "#disable-or-uninstall") Disable or uninstall
...
| Variable | Effect |
| --- | --- |
| `ENABLE_PATTERN_RULES=0` | Disable the [per-edit pattern check](#on-each-file-edit "#on-each-file-edit") |
| `ENABLE_STOP_REVIEW=0` | Disable the [end-of-turn diff review](#at-the-end-of-each-turn "#at-the-end-of-each-turn") |
| `ENABLE_COMMIT_REVIEW=0` | Disable the [commit and push review](#on-each-commit-or-push-claude-makes "#on-each-commit-or-push-claude-makes") |
...
If the plugin was enabled through a project’s `.claude/settings.json`, disabling it from `/plugin` writes an override to your `.claude/settings.local.json` rather than editing the checked-in file, so the plugin stays off for you while teammates are unaffected. The same dialog also offers to uninstall the plugin for everyone by removing it from the shared `.claude/settings.json`; that option requires Claude Code v2.1.203 or later. If it was enabled through [managed settings](./admin-setup "._admin-setup".md), only an administrator can disable it.

## [​](#how-the-plugin-integrates-with-claude-code "#how-the-plugin-integrates-with-claude-code") How the plugin integrates with Claude Code

The plugin is built entirely on [hooks](./hooks "._hooks".md), the mechanism for running your own code at specific points in Claude’s loop. It registers:
...
If you build your own hooks, the [plugin’s source](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/security-guidance "https://github.com/anthropics/claude-plugins-official/tree/main/plugins/security-guidance") is a working example of running a separate model call from a hook and feeding the result back to the session.

## [​](#how-this-fits-with-other-security-tools "#how-this-fits-with-other-security-tools") How this fits with other security tools
...
| On demand | [`/security-review`](./commands#all-commands "._commands#all-commands".md) | One-time security pass on the current branch, run when you ask |
| On pull request | [Code Review](./code-review "._code-review".md), Team and Enterprise plans | Multi-agent correctness and security review with full codebase context |
...
* [Code Review](./code-review "._code-review".md): set up the PR-time multi-agent review
* [Automate actions with hooks](./hooks-guide "._hooks-guide".md): build your own checks at the same lifecycle points
* [Discover and install plugins](./discover-plugins#official-anthropic-marketplace "._discover-plugins#official-anthropic-marketplace".md): browse other official plugins