...
* **Trust verification**: First-time codebase runs and new MCP servers require trust verification
  + Note: Trust verification is disabled when running non-interactively with the `-p` flag. The exception is [`--worktree`](./overview.md), which still requires that trust has been accepted for the directory
