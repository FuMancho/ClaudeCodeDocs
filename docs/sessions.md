...
## [​](#resume-a-session "#resume-a-session") Resume a session

Sessions are saved continuously to [local transcript files](#export-and-locate-session-data "#export-and-locate-session-data") as you work, so you can return to one after exiting or running `/clear`. Use these entry points:


| Command | What it does |
| --- | --- |
| `claude --continue` | Resumes the most recent session in the current directory |
| `claude --resume` | Opens the [session picker](#use-the-session-picker "#use-the-session-picker") |
...
* Model: the session continues on the model it was using. The model isn’t restored when it has been retired or isn’t allowed by `availableModels`, when a `--model` flag or `ANTHROPIC_MODEL`-family environment variable picks one at launch, or on providers that use provider-specific deployment IDs, such as [Amazon Bedrock, Google Cloud’s Agent Platform, and Microsoft Foundry](./third-party-integrations "._third-party-integrations".md); see [model configuration](./model-config#setting-your-model "._model-config#setting-your-model".md) for the resolution order.
* Permission mode: the mode the session was in. `plan` and `bypassPermissions` are never restored; [bypassing permissions](./permission-modes#skip-all-checks-with-bypasspermissions-mode "._permission-modes#skip-all-checks-with-bypasspermissions-mode".md) must be enabled again at launch, with one of its launch flags or `permissions.defaultMode: "bypassPermissions"` in [settings](./settings#permission-settings "._settings#permission-settings".md). `auto` is restored only when your account still meets the [auto mode requirements](./permission-modes#eliminate-prompts-with-auto-mode "._permission-modes#eliminate-prompts-with-auto-mode".md). Pass `--permission-mode` to override the restored mode.
* Active goal: a [goal](./goal#resume-with-an-active-goal "._goal#resume-with-an-active-goal".md) that was still active when the session ended carries over; its turn count, timer, and token-spend baseline reset.
* Scheduled tasks: [tasks that haven’t expired](./scheduled-tasks#limitations "._scheduled-tasks#limitations".md) are restored. Background Bash and monitor tasks aren’t.
...
Sessions whose first prompt was a [`/loop`](./scheduled-tasks#run-a-prompt-repeatedly-with-%2Floop "._scheduled-tasks#run-a-prompt-repeatedly-with-%2Floop".md) command don’t appear in the picker; running `/loop` later in a conversation doesn’t hide the session. Before v2.1.211, a `/loop` run early in a conversation hid the session from the picker permanently.
From v2.1.169, moving a session with [`/cd`](./commands "._commands".md) relocates it to the new directory’s project storage, so it appears in that directory’s picker afterward. As of v2.1.196, a moved session stays out of the old directory’s picker even after a crash or forced exit. On earlier versions, it could also reappear in the old directory’s list after an exit that wasn’t clean when the old path contained special characters such as underscores.
...
## [​](#name-your-sessions "#name-your-sessions") Name your sessions
...
Interactive sessions you never name still get a default display name when they start. Requires Claude Code v2.1.196 or later. The default combines the working directory’s name with a two-character suffix, for example `my-app-3f`, and identifies the session in listings of running sessions, such as [agent view](./agent-view "._agent-view".md) and `claude agents --json` output.
...
If you don’t name a session, Claude Code generates a session title for it: a short summary of your first prompt, written by a background request to the small/fast model, normally a Haiku-class model. Naming the session with `--name` or `/rename` replaces the generated title. You see the generated title in the [session picker](#use-the-session-picker "#use-the-session-picker") and in the statusline [`session_name`](./statusline "._statusline".md) field when no name is set; like the default display name, it isn’t a resume handle.

## [​](#use-the-session-picker "#use-the-session-picker") Use the session picker
...
## [​](#branch-a-session "#branch-a-session") Branch a session
...
If you omit the name, Claude Code names the new branch after the first prompt in the conversation. As of v2.1.198 this also applies after [compaction](./how-claude-code-works#when-context-fills-up "._how-claude-code-works#when-context-fills-up".md); earlier versions fell back to the literal name `Branched conversation` instead of looking past the compaction summary to the original first prompt.
...
* **`/clear`**: start fresh with an empty context. Claude Code saves the previous conversation; resume it with `/resume`, or, in the same Claude Code process, from [the rewind menu’s previous-session entry](./checkpointing#rewind-past-a-cleared-conversation "._checkpointing#rewind-past-a-cleared-conversation".md). You keep a name you set with `--name` or `/rename` in the new conversation, but not an AI-generated session title
...
### [​](#access-conversations-from-scripts "#access-conversations-from-scripts") Access conversations from scripts
...
* **Run Claude once and capture the result**: invoke `claude -p` with [`--output-format json` or `stream-json`](./headless#get-structured-output "._headless#get-structured-output".md) to capture the result, session ID, usage, and cost of a non-interactive run as structured JSON.
* **Ask an existing session a question**: pass a session ID to [`claude -p --resume`](./headless#continue-conversations "._headless#continue-conversations".md) to send a follow-up prompt, such as a summary request, and capture the structured response.
* **React to session events**: read the `transcript_path` field that [hooks](./hooks#common-input-fields "._hooks#common-input-fields".md) and [status line commands](./statusline#available-data "._statusline#available-data".md) receive as input. A `SessionEnd` hook can archive the transcript when a session ends.
* **Embed Claude in a TypeScript or Python app**: use the [Agent SDK](./agent-sdk_overview "._agent-sdk_overview".md) to receive each message programmatically.
...
### [​](#where-transcripts-are-stored "#where-transcripts-are-stored") Where transcripts are stored

By default, transcripts are stored as JSONL at `~/.claude/projects/<project>/<session-id>.jsonl`, where `<project>` is your working directory path with non-alphanumeric characters replaced by `-`. Each line is a JSON object for a message, tool use, or metadata entry. The entry format is internal to Claude Code and changes between versions, so scripts that parse these files directly can break on any release. To build on session data, use `/export` or the [script interfaces](#access-conversations-from-scripts "#access-conversations-from-scripts") instead.
The location, retention, and write behavior are configurable:


| To | Set | Where |
| --- | --- | --- |
| Move storage off `~/.claude` | [`CLAUDE_CONFIG_DIR`](./env-vars "._env-vars".md) | Environment variable |
| Change the 30-day retention | [`cleanupPeriodDays`](./settings#available-settings "._settings#available-settings".md) | `settings.json` |
| Suppress transcript writes in all modes | [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](./env-vars "._env-vars".md) | Environment variable |
| Suppress writes for one non-interactive run | [`--no-session-persistence`](./cli-reference "._cli-reference".md) | CLI flag with `claude -p` |
