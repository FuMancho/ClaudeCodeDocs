...
Sessions created with [`claude -p`](./headless.md) or the [Agent SDK](./overview.md) do not appear in the session picker, but you can still resume one by passing its session ID to `claude --resume <session-id>`.
