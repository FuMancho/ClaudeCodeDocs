...
| Stateless task, don’t want anything written to disk (TypeScript only) | Set [`persistSession: false`](./typescript.md#options). The session exists only in memory for the duration of the call. Python always persists to disk. |

### [​](#continue-resume-and-fork #continue-resume-and-fork) Continue, resume, and fork

Continue, resume, and fork are option fields you set on `query()` ([`ClaudeAgentOptions`](./python.md#claude-agent-options) in Python, [`Options`](./typescript.md#options) in TypeScript).
...
[`ClaudeSDKClient`](./python.md#claude-sdk-client) handles session IDs internally. Each call to `client.query()` automatically continues the same session. Call [`client.receive_response()`](./python.md#claude-sdk-client) to iterate over the messages for the current query. The client must be used as an async context manager.
...
See the [Python SDK reference](./python.md#choosing-between-query-and-claude-sdk-client) for details on when to use `ClaudeSDKClient` vs the standalone `query()` function.
...
There’s also a [V2 preview](./typescript-v2-preview.md) of the TypeScript SDK that provides `createSession()` with a `send` / `stream` pattern, closer to Python’s `ClaudeSDKClient` in feel. V2 is unstable and its APIs may change; the rest of this documentation uses the stable V1 `query()` function.
...
* **Recover from a limit.** The first run ended with `error_max_turns` or `error_max_budget_usd` (see [Handle the result](./agent-loop.md#handle-the-result)); resume with a higher limit.
...
Both SDKs expose functions for enumerating sessions on disk and reading their messages: [`listSessions()`](./typescript.md#list-sessions) and [`getSessionMessages()`](./typescript.md#get-session-messages) in TypeScript, [`list_sessions()`](./python.md#list-sessions) and [`get_session_messages()`](./python.md#get-session-messages) in Python. Use them to build custom session pickers, cleanup logic, or transcript viewers.
Both SDKs also expose functions for looking up and mutating individual sessions: [`get_session_info()`](./python.md#get-session-info), [`rename_session()`](./python.md#rename-session), and [`tag_session()`](./python.md#tag-session) in Python, and [`getSessionInfo()`](./typescript.md#get-session-info), [`renameSession()`](./typescript.md#rename-session), and [`tagSession()`](./typescript.md#tag-session) in TypeScript. Use them to organize sessions by tag or give them human-readable titles.
...
* [File checkpointing](./file-checkpointing.md): Track and revert file changes across sessions
* [Python `ClaudeAgentOptions`](./python.md#claude-agent-options): Full session option reference for Python
