...
| `deny` | Array of permission rules to deny tool use. Use this to exclude sensitive files from Claude Code access. See [Permission rule syntax](#permission-rule-syntax #permission-rule-syntax) and [Bash permission limitations](./permissions.md#tool-specific-permission-rules) | `[ "WebFetch", "Bash(curl *)", "Read(./.env)", "Read(./secrets/**)" ]` |
...
1. **Managed settings** ([server-managed](./server-managed-settings.md), [MDM/OS-level policies](#configuration-scopes #configuration-scopes), or [managed settings](./settings.md#settings-files))
