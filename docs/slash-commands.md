...
Both you and Claude can pass arguments when invoking a skill. Arguments are available via the `$ARGUMENTS` variable.
This skill fixes a GitHub issue by number. The `$ARGUMENTS` variable gets replaced with whatever follows the skill name:
...
The `!`command“ syntax runs shell commands before the skill content is sent to Claude. The command output replaces the variable, so Claude receives actual data, not the command itself.
...
2. The output replaces the variable in the skill content
