...
For built-in commands like `/help` and `/compact`, and bundled skills like `/debug` and `/simplify`, see the [commands reference](./overview.md).**Custom commands have been merged into skills.** A file at `.claude/commands/deploy.md` and a skill at `.claude/skills/deploy/SKILL.md` both create `/deploy` and work the same way. Your existing `.claude/commands/` files keep working. Skills add optional features: a directory for supporting files, frontmatter to [control whether you or Claude invokes them](#control-who-invokes-a-skill), and the ability for Claude to load them automatically when relevant.
...
Bundled skills are listed alongside built-in commands in the [commands reference](./overview.md), marked **Skill** in the Purpose column.
...
| `$name` | Named argument declared in the [`arguments`](#frontmatter-reference) frontmatter list. Names map to positions in order, so with `arguments: [issue, branch]` the markers `$issue` expands to the first argument and `$branch` to the second. |
...
Indexed arguments use shell-style quoting, so wrap multi-word values in quotes to pass them as a single argument. For example, `/my-skill "hello world" second` makes `$0` expand to `hello world` and `$1` to `second`. The `$ARGUMENTS` markers always expands to the full argument string as typed.
...
Both you and Claude can pass arguments when invoking a skill. Arguments are available via the `$ARGUMENTS` markers.
This skill fixes a GitHub issue by number. The `$ARGUMENTS` markers gets replaced with whatever follows the skill name:
...
The `` !`<command>` `` syntax runs shell commands before the skill content is sent to Claude. The command output replaces the markers, so Claude receives actual data, not the command itself.
...
2. The output replaces the markers in the skill content
3. Claude receives the fully-rendered prompt with actual PR data

This is preprocessing, not something Claude executes. Claude only sees the final result.
Substitution runs once over the original file. Command output is inserted as plain text and is not re-scanned for further `` !`<command>` `` markers, so a command cannot emit a markers for a later pass to expand.
...
* **[Debug your configuration](./overview.md)**: diagnose why a skill isn’t appearing or triggering
...
* **[Commands](./overview.md)**: reference for built-in commands and bundled skills
