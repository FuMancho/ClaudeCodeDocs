...
Claude Code includes several built-in subagents like **Explore**, **Plan**, and **general-purpose**. You can also create custom subagents to handle specific tasks. This page covers the [built-in subagents](#built-in-subagents #built-in-subagents), [how to create your own](#quickstart-create-your-first-subagent #quickstart-create-your-first-subagent), [full configuration options](#configure-subagents #configure-subagents), [patterns for working with subagents](#work-with-subagents #work-with-subagents), and [example subagents](#example-subagents #example-subagents).
...
| `tools` | No | [Tools](#available-tools #available-tools) the subagent can use. Inherits all tools if omitted |
| `disallowedTools` | No | Tools to deny, removed from inherited or specified list |
| `model` | No | [Model](#choose-a-model #choose-a-model) to use: `sonnet`, `opus`, `haiku`, a full model ID (for example, `claude-opus-4-6`), or `inherit`. Defaults to `inherit` |
| `permissionMode` | No | [Permission mode](#permission-modes #permission-modes): `default`, `acceptEdits`, `auto`, `dontAsk`, `bypassPermissions`, or `plan` |
...
| `hooks` | No | [Lifecycle hooks](#define-hooks-for-subagents #define-hooks-for-subagents) scoped to this subagent |
...
Subagents cannot spawn other subagents. If your workflow requires nested delegation, use [Skills](./skills.md) or [chain subagents](#chain-subagents #chain-subagents) from the main conversation.
