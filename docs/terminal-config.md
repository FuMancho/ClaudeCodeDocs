...
Apply only in [fullscreen rendering mode](./overview.md), where messages have a background fill.
...
When you paste more than 10,000 characters into the prompt, Claude Code collapses the input to a `[Pasted text]` markers so the input box stays usable. The full content is still sent to Claude when you submit.
...
* [Fullscreen rendering](./overview.md): details on scrolling, search, and copy in fullscreen mode
