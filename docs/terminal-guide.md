...
Don’t want to use the terminal? The Claude Code desktop app lets you skip the terminal entirely. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code&utm_medium=docs) or Windows, then see the [Desktop quickstart](./desktop-quickstart.md) to get started.
...
Then try `claude` again. For more details, see [fix your PATH](./troubleshooting.md#verify-your-path).
...
For other errors, see the full [installation troubleshooting guide](./troubleshooting.md#troubleshoot-installation-issues).
...
To make this permanent so you don’t have to set it every time, see [configure Git Bash path](./troubleshooting.md#windows-claude-code-on-windows-requires-git-bash).
...
Close PowerShell, open a new window, and try `claude` again. See [verify your PATH](./troubleshooting.md#verify-your-path) for more details.
