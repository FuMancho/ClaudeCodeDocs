...
* The [web](/docs/en/claude-code-on-the-web) at claude.ai/code for remote sessions
...
source ~/.bashrc
```
Then try `claude` again. For more details, see [fix your PATH](/docs/en/troubleshooting#verify-your-path).
...
```
For other errors, see the full [installation troubleshooting guide](/docs/en/troubleshooting#troubleshoot-installation-issues).
...
To make this permanent so you don’t have to set it every time, see [configure Git Bash path](/docs/en/troubleshooting#windows-claude-code-on-windows-requires-git-bash).
...
```
Close PowerShell, open a new window, and try `claude` again. See [verify your PATH](/docs/en/troubleshooting#verify-your-path) for more details.
