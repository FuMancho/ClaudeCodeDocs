...
| Symptom | Go to |
| --- | --- |
| `command not found`, install fails, PATH issues, `EACCES`, TLS errors | [Troubleshoot installation and login](./overview.md) |
| Login loops, OAuth errors, `403 Forbidden`, “organization disabled”, Bedrock/Vertex/Foundry credentials | [Troubleshoot installation and login](./overview.md#login-and-authentication) |
| Settings not applying, hooks not firing, MCP servers not loading | [Debug your configuration](./overview.md) |
| `API Error: 5xx`, `529 Overloaded`, `429`, request validation errors | [Error reference](./overview.md) |
| `model not found` or `you may not have access to it` | [Error reference](./overview.md#theres-an-issue-with-the-selected-model) |
...
Then set `USE_BUILTIN_RIPGREP=0` in your [environment](./overview.md).
