...
If you’d rather skip the terminal entirely, the [Claude Code Desktop app](./desktop-quickstart.md) lets you install and use Claude Code through a graphical interface. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code&utm_medium=docs) or Windows and start coding without any command-line setup.
