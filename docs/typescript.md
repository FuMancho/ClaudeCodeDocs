**Try the new V2 interface (preview):** A simplified interface with `send()` and `stream()` patterns is now available, making multi-turn conversations easier. [Learn more about the TypeScript V2 preview](./typescript-v2-preview.md)
...
| --- | --- | --- |
| `prompt` | `string | AsyncIterable<`[`SDKUserMessage`](#sdkuser-message #sdkuser-message)`>` | The input prompt as a string or async iterable for streaming mode |
| `options` | [`Options`](#options #options) | Optional configuration object (see Options type below) |

#### [​](#returns #returns) Returns

Returns a [`Query`](#query-object #query-object) object that extends `AsyncGenerator<`[`SDKMessage`](#sdk-message #sdk-message)`, void>` with additional methods.
...
| `handler` | `(args, extra) => Promise<`[`CallToolResult`](#call-tool-result #call-tool-result)`>` | Async function that executes the tool logic |
| `extras` | `{ annotations?:` [`ToolAnnotations`](#tool-annotations #tool-annotations) `}` | Optional MCP tool annotations providing behavioral hints to clients |
...
| `tag` | `string | undefined` | User-set session tag (see [`tagSession()`](#tag-session #tag-session)) |
...
Returns [`SDKSessionInfo`](#return-type-sdk-session-info #return-type-sdk-session-info), or `undefined` if the session is not found.
...
| `allowedTools` | `string[]` | `[]` | Tools to auto-approve without prompting. This does not restrict Claude to only these tools; unlisted tools fall through to `permissionMode` and `canUseTool`. Use `disallowedTools` to block tools. See [Permissions](./permissions.md#allow-and-deny-rules) |
| `betas` | [`SdkBeta`](#sdk-beta #sdk-beta)`[]` | `[]` | Enable beta features |
| `canUseTool` | [`CanUseTool`](#can-use-tool #can-use-tool) | `undefined` | Custom permission function for tool usage |
...
| `hooks` | `Partial<Record<`[`HookEvent`](#hook-event #hook-event)`,` [`HookCallbackMatcher`](#hook-callback-matcher #hook-callback-matcher)`[]>>` | `{}` | Hook callbacks for events |
...
| `permissionMode` | [`PermissionMode`](#permission-mode #permission-mode) | `'default'` | Permission mode for the session |
...
| `plugins` | [`SdkPluginConfig`](#sdk-plugin-config #sdk-plugin-config)`[]` | `[]` | Load custom plugins from local paths. See [Plugins](./plugins.md) for details |
...
| `sandbox` | [`SandboxSettings`](#sandbox-settings #sandbox-settings) | `undefined` | Configure sandbox behavior programmatically. See [Sandbox settings](#sandbox-settings #sandbox-settings) for details |
| `sessionId` | `string` | Auto-generated | Use a specific UUID for the session instead of auto-generating one |
| `settingSources` | [`SettingSource`](#setting-source #setting-source)`[]` | `[]` (no settings) | Control which filesystem settings to load. When omitted, no settings are loaded. **Note:** Must include `'project'` to load CLAUDE.md files |
...
| `thinking` | [`ThinkingConfig`](#thinking-config #thinking-config) | `{ type: 'adaptive' }` for supported models | Controls Claude’s thinking/reasoning behavior. See [`ThinkingConfig`](#thinking-config #thinking-config) for options |
| `toolConfig` | [`ToolConfig`](#tool-config #tool-config) | `undefined` | Configuration for built-in tool behavior. See [`ToolConfig`](#tool-config #tool-config) for details |
...
| `supportedAgents()` | Returns available subagents as [`AgentInfo`](#agent-info #agent-info)`[]` |
...
| `suggestions` | [`PermissionUpdate`](#permission-update #permission-update)`[]` | Suggested permission updates so the user is not prompted again for this tool |
...
See [`McpServerConfig`](#mcp-server-config #mcp-server-config) for details on each transport type.
...
| `network` | [`SandboxNetworkConfig`](#sandbox-network-config #sandbox-network-config) | `undefined` | Network-specific sandbox configuration |
| `filesystem` | [`SandboxFilesystemConfig`](#sandbox-filesystem-config #sandbox-filesystem-config) | `undefined` | Filesystem-specific sandbox configuration for read/write restrictions |
