...
Dictation also works in [agent view](./agent-view#peek-and-reply "._agent-view#peek-and-reply".md). Hold or tap your push-to-talk key while the dispatch input or a peek-panel reply is focused to dictate to a background session.
...
* **A local microphone**: voice dictation does not work in remote environments such as [Claude Code on the web](./claude-code-on-the-web "._claude-code-on-the-web".md) or SSH sessions.
...
Transcription does not consume Claude messages or tokens and does not count toward the limits shown in `/usage`. See [data usage](./data-usage "._data-usage".md) for how Anthropic handles your data.
...
The Claude Code [VS Code extension](./vs-code "._vs-code".md) also supports voice dictation with the same Claude.ai account requirement. It is not available in VS Code Remote sessions, including SSH, Dev Containers, and Codespaces, because the microphone is on your local machine and the extension runs on the remote host.

## [​](#enable-voice-dictation "#enable-voice-dictation") Enable voice dictation
...
| `/voice hold` | Enable in [hold mode](#hold-to-record "#hold-to-record") |
| `/voice tap` | Enable in [tap mode](#tap-to-record-and-send "#tap-to-record-and-send") |
| `/voice off` | Disable |

Voice dictation persists across sessions. Set it directly in your [user settings file](./settings "._settings".md) instead of running `/voice`:
...
While voice dictation is enabled, the input footer shows a `hold space to speak` hint when the prompt is empty. The hint reflects your current `voice:pushToTalk` binding and updates if you [rebind the dictation key](#rebind-the-dictation-key "#rebind-the-dictation-key"). The hint text is the same in both modes, and it does not appear if you have a [custom status line](./statusline "._statusline".md) configured.
...
## [​](#hold-to-record "#hold-to-record") Hold to record
...
To skip the warmup, switch to [tap mode](#tap-to-record-and-send "#tap-to-record-and-send") with `/voice tap`, or [rebind to a modifier combination](#rebind-the-dictation-key "#rebind-the-dictation-key") like `meta+k`. Modifier combos start recording on the first keypress.
...
## [​](#tap-to-record-and-send "#tap-to-record-and-send") Tap to record and send
...
## [​](#change-the-dictation-language "#change-the-dictation-language") Change the dictation language

Voice dictation uses the same [`language` setting](./settings "._settings".md) that controls Claude’s response language. If that setting is empty, dictation defaults to English. In the VS Code extension, if `language` is empty, dictation uses VS Code’s `accessibility.voice.speechLanguage` setting before defaulting to English.
...
Set the language in `/config` or directly in settings. You can use either the [BCP 47 language code](https://en.wikipedia.org/wiki/IETF_language_tag "https://en.wikipedia.org/wiki/IETF_language_tag") or the language name:
...
## [​](#rebind-the-dictation-key "#rebind-the-dictation-key") Rebind the dictation key

The dictation key is bound to `voice:pushToTalk` in the `Chat` context and defaults to `Space`. The same binding controls both hold and tap modes. Rebind it in [`~/.claude/keybindings.json`](./keybindings "._keybindings".md):
...
Some keys are not delivered to terminal applications and can’t be bound at all. For example, `Caps Lock` shows an error if you try to bind it. See [customize keyboard shortcuts](./keybindings "._keybindings".md) for the full keybinding syntax and the list of reserved shortcuts.
...
* **`Voice mode is disabled by your organization's policy`**: your organization’s compliance configuration disables voice dictation, as described in [Requirements](#requirements "#requirements"). Contact your organization administrator to confirm whether voice dictation is available for your organization.
...
* **`No speech detected`**: audio reached the transcription service but no words were recognized. Speak closer to the microphone, reduce background noise, and confirm your [dictation language](#change-the-dictation-language "#change-the-dictation-language") matches the language you are speaking.
* **Transcription is garbled or in the wrong language**: dictation defaults to English. If you are dictating in another language, set it in `/config` first. See [Change the dictation language](#change-the-dictation-language "#change-the-dictation-language").

### [​](#terminal-not-listed-in-macos-microphone-settings "#terminal-not-listed-in-macos-microphone-settings") Terminal not listed in macOS Microphone settings
...
* [Customize keyboard shortcuts](./keybindings "._keybindings".md): rebind `voice:pushToTalk` and other CLI keyboard actions
* [Configure settings](./settings "._settings".md): full reference for `voice`, `language`, and other settings keys
* [Interactive mode](./interactive-mode "._interactive-mode".md): keyboard shortcuts, input modes, and session controls
* [Commands](./commands "._commands".md): reference for `/voice`, `/config`, and all other commands