...
Releases [v2.1.114 → v2.1.119](./changelog.md#2-1-114 "/docs/en/changelog#2-1-114")4 features · April 20–24
...
[Ultrareview guide](./ultrareview.md "/docs/en/ultrareview")
...
[Interactive mode: session recap](./interactive-mode.md#session-recap "/docs/en/interactive-mode#session-recap")
...
[Terminal config: create a custom theme](./terminal-config.md#create-a-custom-theme "/docs/en/terminal-config#create-a-custom-theme")
...
[Claude Code on the web](./claude-code-on-the-web.md "/docs/en/claude-code-on-the-web")

Other wins

[Vim visual mode](./interactive-mode.md#vim-editor-mode "/docs/en/interactive-mode#vim-editor-mode"): press `v` for character selection or `V` for line selection in the prompt input, with operators and visual feedback

Hooks can now call MCP tools directly via [`type: “mcp_tool”`](./hooks.md#mcp-tool-hook-fields "/docs/en/hooks#mcp-tool-hook-fields"), so a hook can hit an already-connected server without spawning a process

`/cost` and `/stats` are merged into [`/usage`](./commands.md "/docs/en/commands"); the old names still work as typing shortcuts that open the relevant tab

`/config` changes (theme, editor mode, verbose, and similar) now persist to `~/.claude/settings.json` and follow the same project/local/policy precedence as other [settings](./settings.md "/docs/en/settings")

[Forked subagents](./sub-agents.md#fork-the-current-conversation "/docs/en/sub-agents#fork-the-current-conversation") can be enabled on external builds with `CLAUDE_CODE_FORK_SUBAGENT=1`: a fork inherits your full conversation context instead of starting fresh

Default [effort level](./model-config.md#adjust-effort-level "/docs/en/model-config#adjust-effort-level") for Pro and Max subscribers on Opus 4.6 and Sonnet 4.6 is now `high` (was `medium`)
...
Auto mode: include `“$defaults”` in [`autoMode.allow`, `soft_deny`, or `environment`](./auto-mode-config.md "/docs/en/auto-mode-config") to add custom rules alongside the built-in list instead of replacing it

New [`claude plugin tag`](./plugin-dependencies.md#tag-plugin-releases-for-version-resolution "/docs/en/plugin-dependencies#tag-plugin-releases-for-version-resolution") command creates release git tags for plugins with version validation
...
[Full changelog for v2.1.114–v2.1.119 →](./changelog.md#2-1-114 "/docs/en/changelog#2-1-114")