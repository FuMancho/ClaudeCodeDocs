...
Releases [v2.1.143 → v2.1.149](./changelog#2-1-143 "._changelog#2-1-143".md)1 feature · May 18–22
...
[Auto mode requirements](./permission-modes#eliminate-prompts-with-auto-mode "._permission-modes#eliminate-prompts-with-auto-mode".md)

Other wins

[`/usage`](./costs#track-your-costs "._costs#track-your-costs".md) now shows a per-category breakdown of what’s driving your plan limits, attributing recent usage to skills, subagents, plugins, and individual MCP servers
...
New [`/code-review`](./code-review "._code-review".md) command reports correctness bugs at a chosen effort level such as `/code-review high`, and `—comment` posts findings as inline GitHub PR comments. `/simplify` remains as a separate cleanup-only review.
...
[Full changelog for v2.1.143–v2.1.149 →](./changelog#2-1-143 "._changelog#2-1-143".md)