...
Releases [v2.1.158 → v2.1.165](./changelog#2-1-158 "._changelog#2-1-158".md)4 features · June 1–5
...
[Auto mode on third-party providers](./permission-modes#enable-auto-mode-on-bedrock-agent-platform-or-foundry "._permission-modes#enable-auto-mode-on-bedrock-agent-platform-or-foundry".md)
...
[Protected paths](./permission-modes#protected-paths "._permission-modes#protected-paths".md)
...
[Plugin commands](./plugins-reference#plugin-list "._plugins-reference#plugin-list".md)
...
[Decide what to enforce](./admin-setup#decide-what-to-enforce "._admin-setup#decide-what-to-enforce".md)

Other wins

The trigger keyword for [dynamic workflows](./workflows "._workflows".md) changed from `workflow` to `ultracode`; asking for a workflow in your own words still works, and the keyword is highlighted in violet in the prompt

[Stop and SubagentStop hooks](./hooks "._hooks".md) can return `hookSpecificOutput.additionalContext` to give Claude feedback and keep the turn going instead of being treated as an error
...
[Full changelog for v2.1.158–v2.1.165 →](./changelog#2-1-158 "._changelog#2-1-158".md)