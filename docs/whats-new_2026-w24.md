...
Releases [v2.1.166 → v2.1.176](./changelog.md#2-1-166)3 features · June 8–12
...
> /agents
```
[Spawn nested subagents](./sub-agents.md#spawn-nested-subagents)
...
claude --safe-mode
```
[Test against a clean configuration](./debug-your-config.md#test-against-a-clean-configuration)

Other wins

[`fallbackModel`](./model-config.md#fallback-model-chains) configures up to three fallback models tried in order when the primary is overloaded or unavailable, and `--fallback-model` now applies to interactive sessions too
