...
Releases [v2.1.166 → v2.1.176](./changelog#2-1-166 "._changelog#2-1-166".md)3 features · June 8–12
...
[Commands reference](./commands#all-commands "._commands#all-commands".md)
...
[Spawn nested subagents](./sub-agents#spawn-nested-subagents "._sub-agents#spawn-nested-subagents".md)
...
[Test against a clean configuration](./debug-your-config#test-against-a-clean-configuration "._debug-your-config#test-against-a-clean-configuration".md)

Other wins

[`fallbackModel`](./model-config#fallback-model-chains "._model-config#fallback-model-chains".md) configures up to three fallback models tried in order when the primary is overloaded or unavailable, and `--fallback-model` now applies to interactive sessions too
...
[Full changelog for v2.1.166–v2.1.176 →](./changelog#2-1-166 "._changelog#2-1-166".md)