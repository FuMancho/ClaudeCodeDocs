...
Releases [v2.1.207 → v2.1.212](./changelog#2-1-207 "._changelog#2-1-207".md)2 features · July 13–17
...
[Pull live data with MCP connectors](./artifacts#pull-live-data-with-mcp-connectors "._artifacts#pull-live-data-with-mcp-connectors".md)
...
[Turn on screen reader mode](./accessibility#turn-on-screen-reader-mode "._accessibility#turn-on-screen-reader-mode".md)
...
[Auto mode](./permission-modes#enable-auto-mode-on-bedrock-agent-platform-or-foundry "._permission-modes#enable-auto-mode-on-bedrock-agent-platform-or-foundry".md) no longer needs the `CLAUDE_CODE_ENABLE_AUTO_MODE` opt-in on Amazon Bedrock, Google Cloud’s Agent Platform, and Microsoft Foundry; administrators can turn it off with `disableAutoMode`
...
New [corporate launcher](./corporate-launcher "._corporate-launcher".md) support: `CLAUDE_CODE_PROCESS_WRAPPER` or the `processWrapper` setting runs the processes Claude Code starts from its own binary, such as the background service and agent view sessions, through a required wrapper executable

`vimInsertModeRemaps` setting maps two-key insert-mode sequences such as `jj` to Escape in vim mode

`--forward-subagent-text` and `CLAUDE_CODE_FORWARD_SUBAGENT_TEXT` include subagent text and thinking blocks in [stream-json output](./headless "._headless".md)
...
[Full changelog for v2.1.207–v2.1.212 →](./changelog#2-1-207 "._changelog#2-1-207".md)