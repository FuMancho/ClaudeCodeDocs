...
A dynamic workflow is a JavaScript script that orchestrates [subagents](./sub-agents.md "/docs/en/sub-agents") at scale. Claude writes the script for the task you describe, and a runtime executes it in the background while your session stays responsive.
...
* Decide [when to use a workflow](#when-to-use-a-workflow "#when-to-use-a-workflow") instead of subagents or skills
* [Run a bundled workflow](#run-a-bundled-workflow "#run-a-bundled-workflow") with `/deep-research`
* [Have Claude write a workflow](#have-claude-write-a-workflow "#have-claude-write-a-workflow") for your task and save it
* Understand [how a workflow runs](#how-a-workflow-runs "#how-a-workflow-runs") and [manage runs](#manage-runs "#manage-runs")

## [​](#when-to-use-a-workflow "#when-to-use-a-workflow") When to use a workflow

[Subagents](./sub-agents.md "/docs/en/sub-agents"), [skills](./skills.md "/docs/en/skills"), and workflows can all run a multi-step task. The difference is who holds the plan:
...
* [Ask for a workflow](#ask-for-a-workflow-in-your-prompt "#ask-for-a-workflow-in-your-prompt") in your prompt with the word `workflow`, and Claude writes one for the task.
...
If the run does what you wanted, you can [save it as a command](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse") afterward.
...
Ultracode is a Claude Code setting that combines `xhigh` [reasoning effort](./model-config.md#adjust-effort-level "/docs/en/model-config#adjust-effort-level") with automatic workflow orchestration. With it on, Claude plans a workflow for each substantive task instead of waiting for you to ask.
...
Whether you see this prompt depends on your [permission mode](./permission-modes.md "/docs/en/permission-modes"):
...
Your permission mode controls only the launch prompt above. The subagents the workflow spawns always run in `acceptEdits` mode and inherit your [tool allowlist](./settings.md#permission-settings "/docs/en/settings#permission-settings"), regardless of your session’s mode. File edits are auto-approved.
...
Workflows are available in the CLI, the Desktop app, the IDE extensions, [non-interactive mode](./headless.md "/docs/en/headless") with `claude -p`, and the [Agent SDK](./agent-sdk/overview.md "/docs/en/agent-sdk/overview"). The same disable settings apply on every surface.
...
To turn workflows off for your whole organization, set `"disableWorkflows": true` in [managed settings](./server-managed-settings.md "/docs/en/server-managed-settings"), or use the toggle on the [Claude Code admin settings](https://claude.ai/admin-settings/claude-code "https://claude.ai/admin-settings/claude-code") page.
...
* [Run agents in parallel](./agents.md "/docs/en/agents"): compare subagents, agent view, agent teams, and workflows
* [Create custom subagents](./sub-agents.md "/docs/en/sub-agents"): the worker primitive workflows orchestrate
* [Manage costs](./costs.md "/docs/en/costs"): how multi-agent runs count toward usage limits