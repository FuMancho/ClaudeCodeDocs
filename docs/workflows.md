...
Workflows are available in the CLI, the Desktop app, the IDE extensions, [non-interactive mode](./headless.md) with `claude -p`, and the [Agent SDK](./agent-sdk_overview.md). The same disable settings apply on every surface.
