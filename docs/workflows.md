...
A dynamic workflow is a JavaScript script that orchestrates [subagents](./sub-agents "._sub-agents".md) at scale. Claude writes the script for the task you describe, and a runtime executes it in the background while your session stays responsive.
...
## [​](#when-to-use-a-workflow "#when-to-use-a-workflow") When to use a workflow

[Subagents](./sub-agents "._sub-agents".md), [skills](./skills "._skills".md), [agent teams](./agent-teams "._agent-teams".md), and workflows can all run a multi-step task. The difference is who holds the plan:
...
## [​](#run-a-bundled-workflow "#run-a-bundled-workflow") Run a bundled workflow

The quickest way to see a workflow in action is to run `/deep-research`, the [built-in workflow](#bundled-workflows "#bundled-workflows") Claude Code includes for investigating a question across many sources. You’ll see agents work through a set of phases in the background while your session stays free, and get one report at the end instead of a turn-by-turn transcript.
...
Claude Code asks whether to allow the workflow. Select **Yes** to continue. The exact prompt depends on your permission mode. See [Approve the plan before it runs](#approve-the-plan-before-it-runs "#approve-the-plan-before-it-runs") for the per-mode options.
...
To run a workflow for your own task, [have Claude write one](#have-claude-write-a-workflow "#have-claude-write-a-workflow"), and once a run does what you wanted you can [save it](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse") as a command of your own.

### [​](#bundled-workflows "#bundled-workflows") Bundled workflows
...
| --- | --- |
| `/deep-research <question>` | Fans out web searches on a question across several angles, fetches and cross-checks the sources it finds, votes on each claim, and returns a cited report with claims that didn’t survive cross-checking filtered out. Requires the [WebSearch tool](./tools-reference#websearch-tool-behavior "._tools-reference#websearch-tool-behavior".md) to be available |

[Workflows you save](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse") yourself become commands the same way and appear in `/` autocomplete alongside the bundled ones.

### [​](#watch-the-run "#watch-the-run") Watch the run
...
| `s` | [Save](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse") the run’s script as a command |

## [​](#have-claude-write-a-workflow "#have-claude-write-a-workflow") Have Claude write a workflow

You can have Claude write a workflow for your task in two ways:

* [Ask for a workflow](#ask-for-a-workflow-in-your-prompt "#ask-for-a-workflow-in-your-prompt") in your prompt, either in your own words or by including the keyword `ultracode`, and Claude writes one for the task.
* [Let Claude decide with ultracode](#let-claude-decide-with-ultracode "#let-claude-decide-with-ultracode"): set `/effort ultracode` and Claude plans a workflow for every substantive task in the session.

You can also run a workflow command that already exists: a [bundled workflow](#bundled-workflows "#bundled-workflows") like `/deep-research`, or one you’ve [saved](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse").

### [​](#ask-for-a-workflow-in-your-prompt "#ask-for-a-workflow-in-your-prompt") Ask for a workflow in your prompt
...
Claude Code highlights the keyword in your input and Claude writes a workflow script for the task instead of working through it turn by turn. The keyword only chooses how Claude structures the work: a workflow started this way runs inside the session’s existing [permission mode](./permission-modes "._permission-modes".md), and its agents’ tool calls receive the same permission checks and [sandboxing](./sandboxing "._sandboxing".md) as any other tool call in the session.
If the run does what you wanted, you can [save it as a command](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse") afterward. If you already have an orchestrator built another way, such as a folder of subagent prompts or a skill that fans work out, you can point Claude at it and ask for a workflow that does the same thing.

#### [​](#dismiss-or-turn-off-the-keyword "#dismiss-or-turn-off-the-keyword") Dismiss or turn off the keyword
...
#### [​](#where-the-keyword-works "#where-the-keyword-works") Where the keyword works

The keyword is an opt-in only in a prompt you type yourself: at the interactive prompt, in an IDE extension panel, in a [Remote Control](./remote-control "._remote-control".md) client, or in an Agent SDK application that stamps your keyboard input’s [`origin`](./agent-sdk_typescript#sdkmessageorigin "._agent-sdk_typescript#sdkmessageorigin".md) as `{ kind: "human" }`. It doesn’t start a workflow when it reaches the session another way:
...
### [​](#let-claude-decide-with-ultracode "#let-claude-decide-with-ultracode") Let Claude decide with ultracode

Ultracode is a Claude Code setting that combines `xhigh` [reasoning effort](./model-config#adjust-effort-level "._model-config#adjust-effort-level".md) with automatic workflow orchestration. With it on, Claude plans a workflow for each substantive task instead of waiting for you to ask.
...
Ultracode lasts for the current session and resets when you start a new one. Drop back with `/effort high` when you return to routine work. It’s available on models that support `xhigh` [effort](./model-config#adjust-effort-level "._model-config#adjust-effort-level".md); on other models the `/effort` menu doesn’t offer it.

### [​](#approve-the-plan-before-it-runs "#approve-the-plan-before-it-runs") Approve the plan before it runs
...
Whether you see this prompt depends on your [permission mode](./permission-modes "._permission-modes".md):
...
Your permission mode controls only the launch prompt above. The subagents the workflow spawns always run in `acceptEdits` mode and inherit your [tool allowlist](./settings#permission-settings "._settings#permission-settings".md), regardless of your session’s mode. File edits are auto-approved.
...
### [​](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse") Save the workflow for reuse
...
* `~/.claude/workflows/` in your home directory: available in every project, visible only to you. If you set [`CLAUDE_CONFIG_DIR`](./env-vars "._env-vars".md), this location is the `workflows/` directory under that path.
...
### [​](#pass-input-to-a-saved-workflow "#pass-input-to-a-saved-workflow") Pass input to a saved workflow
...
## [​](#example-workflow-prompts "#example-workflow-prompts") Example workflow prompts
...
### [​](#audit-many-files-for-the-same-issue "#audit-many-files-for-the-same-issue") Audit many files for the same issue
...
### [​](#keep-fixing-until-a-check-passes "#keep-fixing-until-a-check-passes") Keep fixing until a check passes
...
### [​](#migrate-many-files-in-parallel "#migrate-many-files-in-parallel") Migrate many files in parallel
...
### [​](#review-every-changed-file-and-write-one-summary "#review-every-changed-file-and-write-one-summary") Review every changed file and write one summary
...
### [​](#research-a-topic-across-many-sources "#research-a-topic-across-many-sources") Research a topic across many sources
...
### [​](#find-issues-until-the-list-stops-growing "#find-issues-until-the-list-stops-growing") Find issues until the list stops growing
...
### [​](#what-the-saved-script-looks-like "#what-the-saved-script-looks-like") What the saved script looks like

When you [save a workflow](#save-the-workflow-for-reuse "#save-the-workflow-for-reuse"), the file in `.claude/workflows/` holds a `meta` block followed by a script body that orchestrates subagents. You usually don’t need to edit it, but here is the shape of a small one so you can recognize what Claude generated:
...
The runtime tracks each agent’s result as the run progresses, which is what makes a run [resumable](#resume-after-a-pause "#resume-after-a-pause") within the same session.

### [​](#behavior-and-limits "#behavior-and-limits") Behavior and limits
...
## [​](#manage-runs "#manage-runs") Manage runs
...
### [​](#resume-after-a-pause "#resume-after-a-pause") Resume after a pause
...
To gauge the spend before committing to a large task, run the workflow on a small slice first: one directory instead of the whole repo, or a narrow question instead of a broad one. The `/workflows` view shows each agent’s token usage as the run progresses, and you can stop the run there at any time without losing completed work. The runtime’s [agent caps](#behavior-and-limits "#behavior-and-limits") limit how many agents a single run can spawn, which bounds the cost of a runaway script. To keep every run smaller by default, [set a size guideline](#set-a-size-guideline "#set-a-size-guideline") in `/config`.
Claude Code also flags a run that grows unusually large. When a workflow schedules more than 25 agents, or its projected token total passes 1.5 million, its progress line in the task panel below the input box shows a `Large workflow` warning. The warning points you to [`/workflows`](#watch-the-run "#watch-the-run"), where you can stop the run. Requires Claude Code v2.1.203 or later.
The warning is advisory: it doesn’t pause or limit the run. Two settings change when you see it:

* If you [set a size guideline](#set-a-size-guideline "#set-a-size-guideline"), the guideline’s agent count replaces the 25-agent threshold.
* Sessions with [ultracode](#let-claude-decide-with-ultracode "#let-claude-decide-with-ultracode") on don’t show the warning, because turning ultracode on already opts you in to large runs.

Every agent in a workflow uses your session’s model unless the script routes a stage to a different one or the [`CLAUDE_CODE_SUBAGENT_MODEL`](./model-config#environment-variables "._model-config#environment-variables".md) environment variable is set, which overrides both. To control the model cost:
...
### [​](#set-a-size-guideline "#set-a-size-guideline") Set a size guideline
...
Changes take effect on the next prompt. The [runtime agent caps](#behavior-and-limits "#behavior-and-limits") still apply regardless of the setting.

### [​](#turn-workflows-off "#turn-workflows-off") Turn workflows off

Workflows are available in the CLI, the Desktop app, the IDE extensions, [non-interactive mode](./headless "._headless".md) with `claude -p`, and the [Agent SDK](./agent-sdk_overview "._agent-sdk_overview".md). The same disable settings apply on every surface.
...
To turn workflows off for your whole organization, set `"disableWorkflows": true` in [managed settings](./server-managed-settings "._server-managed-settings".md), or use the toggle on the [Claude Code admin settings](https://claude.ai/admin-settings/claude-code "https://claude.ai/admin-settings/claude-code") page.
...
* [Run agents in parallel](./agents "._agents".md): compare subagents, agent view, agent teams, and workflows
* [Create custom subagents](./sub-agents "._sub-agents".md): the worker primitive workflows orchestrate
* [Manage costs](./costs "._costs".md): how multi-agent runs count toward usage limits