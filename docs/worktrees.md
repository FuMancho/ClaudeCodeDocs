...
Worktrees require a git repository; for other version control systems, [configure hooks to replace the git logic](#non-git-version-control "#non-git-version-control"). In the [desktop app](./desktop#work-in-parallel-with-sessions "._desktop#work-in-parallel-with-sessions".md), every new session gets its own worktree automatically.
...
Most sessions need only the first two sections: [start Claude in a worktree](#start-claude-in-a-worktree "#start-claude-in-a-worktree"), then [clean up when you exit](#clean-up-worktrees "#clean-up-worktrees"). Return to the rest of the page when you need to [resume a session](#resume-a-worktree-session "#resume-a-worktree-session"), [change how worktrees are created](#customize-worktree-creation "#customize-worktree-creation"), or [debug a failure](#troubleshooting "#troubleshooting").
...
Interactive runs require [workspace trust](./security "._security".md): if you haven’t run Claude in the directory before, run `claude` once there to accept the trust dialog, or `--worktree` exits with an error prompting you to. Non-interactive runs with `-p` skip the trust check, so `claude -p --worktree` proceeds without it.

Add `.claude/worktrees/` to your `.gitignore` so worktree contents don’t appear as untracked files in your main checkout.

### [​](#set-up-the-worktree-environment "#set-up-the-worktree-environment") Set up the worktree environment
...
* **The worktree is clean**: for an unnamed session, Claude removes the worktree and its branch automatically. A [named](./sessions#name-your-sessions "._sessions#name-your-sessions".md) session prompts you first so you can keep the worktree for later
...
## [​](#resume-a-worktree-session "#resume-a-worktree-session") Resume a worktree session

When you resume a session that was inside a worktree, Claude Code returns the session to that worktree. This holds for interactive resumes, for `--continue` and `--resume` in [non-interactive mode](./headless "._headless".md) with `-p`, and for the Agent SDK. Back inside the worktree, Claude can still exit it with the [`ExitWorktree`](./tools-reference "._tools-reference".md) tool.
...
When Claude enters or exits a worktree that Claude Code created with git, the transcript follows: Claude Code records the session under the session’s new working directory, the same way [`/cd`](./commands "._commands".md) does, so `/desktop` and `--resume` find it there. Exiting moves it back the same way. A worktree created by a [`WorktreeCreate` hook](#non-git-version-control "#non-git-version-control") keeps its transcript at the launch directory. Requires Claude Code v2.1.198 or later.

## [​](#isolate-subagents-with-worktrees "#isolate-subagents-with-worktrees") Isolate subagents with worktrees

Subagents can run in their own worktrees so parallel edits don’t conflict. Ask Claude to “use worktrees for your agents”, or make the isolation permanent for a [custom subagent](./sub-agents#supported-frontmatter-fields "._sub-agents#supported-frontmatter-fields".md) by adding `isolation: worktree` to its frontmatter.
...
### [​](#clean-up-subagent-and-background-session-worktrees "#clean-up-subagent-and-background-session-worktrees") Clean up subagent and background-session worktrees

A periodic sweep removes worktrees that Claude created for subagents and [background sessions](./agent-view#how-file-edits-are-isolated "._agent-view#how-file-edits-are-isolated".md) once they are older than your [`cleanupPeriodDays`](./settings#available-settings "._settings#available-settings".md) setting. The sweep skips a worktree that still holds work: changed or untracked files, or unpushed commits. It never removes worktrees you create with `--worktree`.
...
## [​](#customize-worktree-creation "#customize-worktree-creation") Customize worktree creation
...
New worktrees branch from the repository’s default branch, so most sessions don’t need this setting. Set `worktree.baseRef` in [settings](./settings#worktree-settings "._settings#worktree-settings".md) to branch from your current work instead. The setting accepts two values:
...
You can’t set `worktree.baseRef` to a branch name. To start a worktree from a specific existing branch, [create it with git directly](#manage-worktrees-manually "#manage-worktrees-manually").
...
### [​](#branch-from-a-pull-request "#branch-from-a-pull-request") Branch from a pull request
...
This applies to every worktree Claude Code creates with git: `--worktree` worktrees, [subagent worktrees](#isolate-subagents-with-worktrees "#isolate-subagents-with-worktrees"), and parallel sessions in the [desktop app](./desktop#work-in-parallel-with-sessions "._desktop#work-in-parallel-with-sessions".md). With a [`WorktreeCreate` hook](#non-git-version-control "#non-git-version-control"), copy the files inside the hook script.

### [​](#reuse-a-worktree-name "#reuse-a-worktree-name") Reuse a worktree name

Passing `--worktree` a name whose directory already exists opens that existing worktree instead of creating a new one.
With the default `"fresh"` [base](#choose-the-base-branch "#choose-the-base-branch"), a reopened worktree resets to the repository’s default branch instead of continuing at its old tip when all of the following hold:
...
### [​](#replace-worktree-creation-with-a-hook "#replace-worktree-creation-with-a-hook") Replace worktree creation with a hook

Configure a [`WorktreeCreate` hook](./hooks#worktreecreate "._hooks#worktreecreate".md) to replace the default `git worktree` logic entirely, including placing worktrees somewhere other than `.claude/worktrees/`. For a complete example, see [Non-git version control](#non-git-version-control "#non-git-version-control").

## [​](#what-worktrees-share-with-the-main-checkout "#what-worktrees-share-with-the-main-checkout") What worktrees share with the main checkout
...
* **The repository’s `.git` directory**: git commands in a worktree write to the main repository’s shared `.git` directory, and [sandboxing](./sandboxing#filesystem-isolation "._sandboxing#filesystem-isolation".md) allows those writes, so commands such as `git commit` work from inside a worktree with the sandbox enabled.
* **Plugins**: plugins installed at [project scope](./plugins-reference#plugin-installation-scopes "._plugins-reference#plugin-installation-scopes".md) from the main checkout also load in worktrees of the same repository, so you don’t need to reinstall them per worktree. Requires Claude Code v2.1.200 or later.
* **Permission approvals**: choosing “Yes, don’t ask again” for a Bash command in a worktree session saves the rule to the main checkout’s `.claude/settings.local.json`, so it applies in the main checkout and in every other worktree of the repository, and it survives the worktree’s removal. Before v2.1.211, an approval granted in a worktree was saved inside that worktree, didn’t apply elsewhere, and was lost when the worktree was removed. See [where approvals are saved](./permissions#permission-system "._permissions#permission-system".md).

All three apply whether you create the worktree with `--worktree`, with `git worktree add`, or through the [desktop app](./desktop#work-in-parallel-with-sessions "._desktop#work-in-parallel-with-sessions".md).
...
See the [Git worktree documentation](https://git-scm.com/docs/git-worktree "https://git-scm.com/docs/git-worktree") for the full command reference.
...
This `WorktreeCreate` hook reads the worktree name from the JSON on stdin with `jq`, checks out a fresh SVN working copy, and prints the directory path so Claude Code can use it as the session’s working directory. Add the configuration to your [`settings.json`](./settings#settings-files "._settings#settings-files".md):
...
### [​](#claude-code-can’t-enter-the-worktree-at-startup "#claude-code-can’t-enter-the-worktree-at-startup") Claude Code can’t enter the worktree at startup

When Claude Code can’t enter the worktree directory at startup, it prints an error naming the path and exits with code 1. This can happen when a [`WorktreeCreate` hook](./hooks#worktreecreate "._hooks#worktreecreate".md) prints something other than the directory it created, or when the directory was deleted after it was set up. Before v2.1.205, this crashed the session, and with `-p` it stalled for about 30 seconds before exiting with code 0.

### [​](#worktree-creation-fails-on-a-symlinked-path "#worktree-creation-fails-on-a-symlinked-path") Worktree creation fails on a symlinked path
