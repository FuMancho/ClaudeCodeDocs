...
To put worktrees somewhere else, configure a [`WorktreeCreate` hook](#non-git-version-control "#non-git-version-control"). Run the command again with a different name in another terminal to start a second isolated session:
...
See the [Git worktree documentation](https://git-scm.com/docs/git-worktree "https://git-scm.com/docs/git-worktree") for the full command reference. Remember to initialize your development environment in each new worktree: install dependencies, set up virtual environments, or run whatever your project’s setup requires.
